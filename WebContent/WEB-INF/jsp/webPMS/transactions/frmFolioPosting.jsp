	...
	$(function() 
			{
				...
				$("#txtDocDate").datepicker({ dateFormat: 'dd-mm-yy' });
				$("#txtDocDate").datepicker('setDate', pmsDate);	
...
//calculate totals
	...
	function funSetFolioEditData(folioNo)
	{
		$("#strFolioNo").val(folioNo);
		var searchUrl=getContextPath()+"/loadFolioEditData.html?folioNo="+folioNo;
		$.ajax({
			...
			{ 
	            ...
	            {
				    $("#strFolioNo").val(item[0]);
					$("#strTransactionId").val(item[1]);
					$("#strIncomeHead").val(item[2])
					$("#lblIncomeHeadName").text(item[3]);
					$("#dblRate").val(item[4]);
					$("#dblQuantity").val(item[5])
					$("#txtDocDate").val(item[6]);
	    ...
	    <div class="col-md-3">
	      <div class="row">
              <div class="col-md-6"><label>Transaction id.</label> 
			       <s:input id="strTransactionId" path="strTransactionId"   readonly="true" ondblclick="funHelp('transactionId')" cssClass="searchTextBox" style="height: 45%;"/>
			   </div>
			...
			<div class="col-md-2"><label id="lbDocDate">Doc Date</label>
			        <s:input type="text" id="txtDocDate" path="dteDocDate" cssClass="calenderTextBox" />
		         </div>
