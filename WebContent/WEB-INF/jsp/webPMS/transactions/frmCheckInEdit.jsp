...
<script type="text/javascript">
	...
	function funSetCheckInData(code){
		...
		$.ajax({
			type : "GET",
			url : getContextPath()+ "/loadCheckInEditData.html?docCode=" + code,
			dataType : "json",
...
//	 	        		$("#lblRoomType").text('');
		...
		/*  if(gTempAdultcount>=$("#txtNoOfAdults").val()){
			...
			//    row.insertCell(8).innerHTML= "<input type=\"button\" class=\"deletebutton\" size=\"6%\" value = \"\" onClick=\"Javacsript:funDeleteRow(this)\"/>";
			    row.insertCell(8).innerHTML= "<input size=\"1%\" name=\"listCheckInDetailsBean["+(rowCount)+"].strGuestCode\" id=\"strGuestCode."+(rowCount)+"\" value='"+guestCode+"' type='hidden' />";
			    row.insertCell(9).innerHTML= "<input readonly=\"readonly\" class=\"Box\" size=\"2%\" id=\"strExtraBedDesc."+(rowCount)+"\" value='"+extraBedDesc+"' />";
			    row.insertCell(10).innerHTML= "<input type=\"hidden\" class=\"Box \" name=\"listCheckInDetailsBean["+(rowCount)+"].strRoomType\" id=\"strRoomType."+(rowCount)+"\" value='"+roomTypeCode+"' >"; 
...
<body>
	<label id="formHeading">Check In Edit</label>
	  <s:form name="frmCheckInEdit" method="POST" action="saveCheckInEdit.html">
	  <br>
	  <div id="multiAccordion">	
		  
     <h3><a href="#">Check In Edit</a></h3>
			<div>
