...
function funShowTableGUI(divID)
{
	...
	document.all["divIncomeHead"].style.display = 'none';
...
function funOnClckIncomeHeadBtn( divId)
{
	funShowTableGUI(divId)
	$('#lblButton').text("Income Head Wise ");
	var frmDte1=$('#dteFromDate').val();
    var toDte1=$('#dteToDate').val();
	var searchUrl=getContextPath()+"/loadIncomeHeadDtl.html?frmDte="+frmDte1+"&toDte="+toDte1;
	$.ajax({
	        type: "GET",
	        url: searchUrl,
		    dataType: "json",
		    success: function(response)
		    {
		    	funIncomeHeadDetail(response);
		    	$("#lblButton").text("Income Head Flash");
...
function funIncomeHeadDetail(ProdDtl)
{
	$('#tblIncomeHeadDet tbody').empty();
	for(var i=0;i<ProdDtl.length;i++)
	{
	 var data=ProdDtl[i];
	 
    var table = document.getElementById("tblIncomeHeadDet");
    var rowCount = table.rows.length;
    var row = table.insertRow(rowCount);
    var folioDocNo=data.strDocCode;
    
    row.insertCell(0).innerHTML= "<input name=\"strFolioNo["+(rowCount)+"]\" readonly=\"readonly\"  class=\"Box\" size=\"15%\" id=\"strFolioNo."+(rowCount)+"\" value='"+data.strFolioNo+"' onclick=\"funIncomeHeadOpenSlip(this,'"+folioDocNo+"')\">";		    
    row.insertCell(1).innerHTML= "<input name=\"dteDocDate["+(rowCount)+"]\" id=\"dteDocDate."+(rowCount)+"\" readonly=\"readonly\" style=\"text-align: right;\" size=\"15%\" class=\"Box\" value="+data.dteDocDate+">";
    row.insertCell(2).innerHTML= "<input name=\"strDocCode["+(rowCount)+"]\" id=\"strDocCode."+(rowCount)+"\" readonly=\"readonly\"   size=\"20%\" class=\"Box\" value="+data.strDocCode+">";
    row.insertCell(3).innerHTML= "<input name=\"strPerticular["+(rowCount)+"]\" id=\"strPerticular."+(rowCount)+"\" readonly=\"readonly\"   size=\"20%\" class=\"Box\" value="+data.strPerticular+">"; 
    row.insertCell(4).innerHTML= "<input name=\"dblQuantity["+(rowCount)+"]\" id=\"dblQuantity."+(rowCount)+"\" readonly=\"readonly\"   size=\"15%\" class=\"Box\" value="+data.dblQuantity+">";
    row.insertCell(5).innerHTML= "<input name=\"dblAmount["+(rowCount)+"]\" readonly=\"readonly\"  class=\"Box\" size=\"15%\" id=\"dblAmount."+(rowCount)+"\" value='"+data.dblAmount+"'>";
    row.insertCell(6).innerHTML= "<input name=\"strUserEdited["+(rowCount)+"]\" id=\"strUserEdited."+(rowCount)+"\" readonly=\"readonly\" style=\"text-align: right;\" size=\"20%\" class=\"Box\" value="+data.strUserEdited+">";
    row.insertCell(7).innerHTML= "<input name=\"dteDateEdited["+(rowCount)+"]\" id=\"dteDateEdited."+(rowCount)+"\" readonly=\"readonly\" style=\"text-align: right;\" size=\"15%\" class=\"Box\" value="+data.dteDateEdited+">";
 
	} 
}

function funIncomeHeadOpenSlip(data,folioDocNo)
{
	
	var folioNo = data.value;
	window.open(getContextPath()+"/rptFolioFostingPrinting.html?folioNo="+folioNo+"&folioDocNo="+folioDocNo+"");
...
function funSetPaymentDetail(ProdDtl,divId)
{
	...
	for(var i=0;i<ProdDtl.length;i++)
	{
     ...
     var checkIndate = data.dteCheckInDate;
     var checkOutdate = data.dteBillDate;
     var roomno = data.strRoomDesc;
     var date = data.dteReceiptDate;
     var guestName = data.strGuestName;
     var against = data.strType;
     var docCode = data.strDocCode;
     ...
     var remark=data.strRemark;
        ...
        row.insertCell(1).innerHTML= "<input name=\"dteCheckInDate["+(rowCount)+"]\" readonly=\"readonly\" class=\"Box\" size=\"10%\" id=\"dteCheckInDate."+(rowCount)+"\" value='"+checkIndate+"'>";
        row.insertCell(2).innerHTML= "<input name=\"dteBillDate["+(rowCount)+"]\" readonly=\"readonly\" class=\"Box\" size=\"10%\" id=\"dteBillDate."+(rowCount)+"\" value='"+checkOutdate+"'>";
        row.insertCell(3).innerHTML= "<input name=\"strRoomDesc["+(rowCount)+"]\" readonly=\"readonly\" class=\"Box\" size=\"10%\" id=\"strRoomDesc."+(rowCount)+"\" value='"+roomno+"'>";
        row.insertCell(4).innerHTML= "<input name=\"dteReceiptDate["+(rowCount)+"]\" readonly=\"readonly\" class=\"Box\" size=\"10%\" id=\"dteReceiptDate."+(rowCount)+"\" value='"+date+"'>";
	    row.insertCell(5).innerHTML= "<input name=\"strGuestName["+(rowCount)+"]\" readonly=\"readonly\" class=\"Box\" size=\"40%\" id=\"strGuestName."+(rowCount)+"\" value='"+guestName+"'>";
	    row.insertCell(6).innerHTML= "<input name=\"strType["+(rowCount)+"]\" readonly=\"readonly\" class=\"Box\" size=\"20%\" id=\"strType."+(rowCount)+"\" value='"+against+"'>";
	    row.insertCell(7).innerHTML= "<input name=\"strType["+(rowCount)+"]\" readonly=\"readonly\" class=\"Box\" size=\"20%\" id=\"strDocCode."+(rowCount)+"\" value='"+docCode+"'>";
	    row.insertCell(8).innerHTML= "<input name=\"strSettlement["+(rowCount)+"]\" readonly=\"readonly\" class=\"Box\" size=\"15%\" id=\"strRoomDesc."+(rowCount)+"\" value='"+settlement+"'>";
	    row.insertCell(9).innerHTML= "<input name=\"dblAmount["+(rowCount)+"]\" readonly=\"readonly\"  style=\"text-align: right;float: right;\" size=\"17%\" class=\"Box\" value="+amt+">";
	    row.insertCell(10).innerHTML= "<input name=\"strRemarks["+(rowCount)+"]\" readonly=\"readonly\" class=\"Box\" size=\"40%\" id=\"strRemarks."+(rowCount)+"\" value='"+remark+"'>";
...
function funSetRefundDetail(ProdDtl)
{
	type="Refund"
	...
	{
	 var data=ProdDtl[i];
	 var table = document.getElementById("tblRefund");
     var rowCount = table.rows.length;
     var row = table.insertRow(rowCount);
     ...
     var remark=data.strRemark;
        ...
        row.insertCell(0).innerHTML= "<input name=\"strReceiptNo["+(rowCount)+"]\" readonly=\"readonly\" class=\"Box\" size=\"15%\" id=\"strBillNo."+(rowCount)+"\" value='"+receiptNo+"' onclick=\"funOpenSlip1(this,'"+against+"','"+type+"')\" >";	 	    
       	row.insertCell(1).innerHTML= "<input name=\"dteReceiptDate["+(rowCount)+"]\" readonly=\"readonly\" class=\"Box\" size=\"10%\" id=\"dteReceiptDate."+(rowCount)+"\" value='"+date+"'>";
	    row.insertCell(2).innerHTML= "<input name=\"strGuestName["+(rowCount)+"]\" readonly=\"readonly\" class=\"Box\" size=\"40%\" id=\"strGuestName."+(rowCount)+"\" value='"+guestName+"'>";
	    row.insertCell(3).innerHTML= "<input name=\"strType["+(rowCount)+"]\" readonly=\"readonly\" class=\"Box\" size=\"20%\" id=\"strType."+(rowCount)+"\" value='"+against+"'>";
	    ...
	    row.insertCell(5).innerHTML= "<input name=\"dblAmount["+(rowCount)+"]\" readonly=\"readonly\"  style=\"text-align: right;float: right;\" size=\"17%\" class=\"Box\" value="+amt+">";
	    row.insertCell(6).innerHTML= "<input name=\"strRemarks["+(rowCount)+"]\" readonly=\"readonly\" class=\"Box\" size=\"40%\" id=\"strRemarks."+(rowCount)+"\" value='"+remark+"'>";
...
function funOpenSlip1(data,against,type)
{
	var receiptNumber = data.value;
	if(type=="Refund")
	{
		window.open(getContextPath()+"/rptRefundRecipt.html?reciptNo="+receiptNumber+"&checkAgainst="+against+ "");
...
function funExportReport()
    ...
    var toDte1=$('#dteToDate').val();
	 ...
	 {
		if($('#cmbSelectType').val() == 'Excel')
		{
		    window.location.href = getContextPath()+"/exportExpectedArrWisePMSSalesFlash.html?frmDte="+frmDte1+"&toDte="+toDte1;
		}else{
		   window.location.href = getContextPath()+"/rptExpectedArrWiseDtlInPDF.html?frmDte="+frmDte1+"&toDte="+toDte1;
		}
	...
	else if(hidReportName=='divExpectedDeptList')
	 {
		if($('#cmbSelectType').val() == 'Excel')
		{
			...
			window.location.href = getContextPath()+"/rptExpectedDeptWiseDtlInPDF.html?frmDte="+frmDte1+"&toDte="+toDte1;
	...
	else if(hidReportName=='divCheckInList')
	 {
		...
		if($('#cmbSelectType').val() == 'Excel')
		{
			...
			window.location.href = getContextPath()+"/rptCheckInWisePMSSalesFlashInPDF.html?frmDte="+frmDte1+"&toDte="+toDte1;
	...
	else if(hidReportName=='divCheckOutList')
	{
		if($('#cmbSelectType').val() == 'Excel')
			{
				...
				window.location.href = getContextPath()+"/rptCheckOutWisePMSSalesFlashInPDF.html?frmDte="+frmDte1+"&toDte="+toDte1;
	 ...
	 {
		 var type="Refund";
		 window.location.href = getContextPath()+"/exportPMSRefund.html?frmDte="+frmDte1+"&toDte="+toDte1+"&type="+type;
	 }
	 else if(hidReportName=='divIncomeHead')
	{
		 window.location.href = getContextPath()+"/exportIncomeHeadFlash.html?frmDte="+frmDte1+"&toDte="+toDte1+"&type="+type;
	}
...
function funOnClickRefund( divId)
{
	...
	var frmDte1=$('#dteFromDate').val();
    var toDte1=$('#dteToDate').val();
	var searchUrl=getContextPath()+"/loadRefundForSalesFlash.html?frmDte="+frmDte1+"&toDte="+toDte1;
	$.ajax({
	        type: "GET",
	        url: searchUrl,
		    dataType: "json",
...
/*function funSetHousekeepingSummary(response)
{
 ...
 function funSetHousekeepingSummary(ProdDtl)
 {
 	$('#tblHousekeepingSummary tbody').empty();
 	for(var i=0;i<ProdDtl.length;i++)
 	{
     ...
     row.insertCell(0).innerHTML= "<input name=\"strRoomDesc["+(rowCount)+"]\" readonly=\"readonly\"  class=\"Box\" size=\"15%\" id=\"strRoomDesc."+(rowCount)+"\" value='"+data.strRoomDesc+"'>";		    
     row.insertCell(1).innerHTML= "<input name=\"strRoomStatus["+(rowCount)+"]\" readonly=\"readonly\" class=\"Box\" size=\"32%\" id=\"strRoomStatus."+(rowCount)+"\" value='"+data.strRoomStatus+"'>";
     row.insertCell(2).innerHTML= "<input name=\"strOccupiedStatus["+(rowCount)+"]\" id=\"strOccupiedStatus."+(rowCount)+"\" readonly=\"readonly\"   size=\"14%\" class=\"Box\" value="+data.strOccupiedStatus+">";
     row.insertCell(3).innerHTML= "<input name=\"dteArrivalDate["+(rowCount)+"]\" id=\"dteArrivalDate."+(rowCount)+"\" readonly=\"readonly\"   size=\"14%\" class=\"Box\" value="+data.dteArrivalDate+">";
     row.insertCell(4).innerHTML= "<input name=\"dteDepartureDate["+(rowCount)+"]\" readonly=\"readonly\"  class=\"Box\" size=\"14%\" id=\"dteDepartureDate."+(rowCount)+"\" value='"+data.dteDepartureDate+"'>";
...
<body onload="funOnLoad();">
	...
	<!--  	<div id="divHousekeepingSummary" class="dynamicTableContainer" style="height: 400px;">
		...
		<div id="divHousekeepingSummary" class="dynamicTableContainer" style="height: 400px;">
			<table style="width: 100%; border: #0F0; table-layout: fixed;" class="transTablex col15-center">
				<tr bgcolor="#c0c0c0">
					<td width="7.4%">Room No </td>
					<td width="10%">Room Status</td>
					<td width="6.1%">Occupied Status</td>
					<td width="5%">Arrival Date</td>
					<td width="4.5%">CheckOut Date</td>
				</tr>
			</table>
			<div style="background-color: #fbfafa; border: 1px solid #ccc; display: block; height: 330px; margin: auto; overflow-x: hidden; overflow-y: scroll; width: 100%;">
				<table id="tblHousekeepingSummary" style="width: 100%; border: #0F0; table-layout: fixed;" class="transTablex col15-center">
					<tbody>
						<col style="width: 7.2%">
						<col style="width: 10%">
						<col style="width: 6%">
 ...
 <div id="divMonthWiseSale" class="dynamicTableContainer" style="height: 400px;">
		...
		<div id="divPayment" class="dynamicTableContainer" style="height: 400px;">
			<table style="width: 100%; border: #0F0; table-layout: fixed;" class="transTablex col15-center">
				<tr bgcolor="#c0c0c0">
					<td width="16%">Receipt No </td>
					<td width="13%">Check-In Date</td>
					<td width="12%">Check-Out Date</td>
					<td width="11%">RoomNO</td>
				<!-- <td width="12%">Reser.RNo</td>--> 
					<td width="12%">ReceiptDate</td>
					<td align="center" width="15%">Name</td>
					<td width="12%">Type</td>
					<td width="12%">DocNo</td>
					<td width="15%">Settlement</td>
					<td width="12%">Amount</td>
					<td width="12%">Remark</td>
				</tr>
			</table>
			<div style="background-color: #fbfafa; border: 1px solid #ccc; display: block; height: 330px; margin: auto; overflow-x: hidden; overflow-y: scroll; width: 100%;">
			<table id="tblPayment" style="width: 100%; border: #0F0; table-layout: fixed;overflow: scroll;display:flex" class="transTablex col15-center">
					<tbody>
						...
						<col style="width: 6%"> 
						<col style="width: 6%">
						<col style="width: 6%;/* padding-right: 7%; */">
						<col style="width: 6%">
					</colgroup>
					</tbody>
				</table>
 ...
 --%>						<col style="width: 2%">
    ...
    <div id="divAvailableRooms" class="dynamicTableContainer" style="height: 400px;">
		...
		<div id="divRefund" class="dynamicTableContainer" style="height: 400px;">
			<table style="width: 100%; border: #0F0; table-layout: fixed;" class="transTablex col15-center">
				<tr bgcolor="#c0c0c0">
					<td width="12%">Receipt No </td>
					<td width="12%">ReceiptDate</td>
					<td align="center" width="18%">Name</td>
					<td width="12%">Type</td>
					<td width="15%">Settlement</td>
					<td width="12%">Amount</td>
					<td width="12%">Remark</td>
			...
			<div style="background-color: #fbfafa; border: 1px solid #ccc; display: block; height: 330px; margin: auto; overflow-x: hidden; overflow-y: scroll; width: 100%;">
				<table id="tblRefund" style="width: 100%; border: #0F0; table-layout: fixed;overflow: scroll;display:flex" class="transTablex col15-center">
					<tbody>
						<colgroup><col style="width: 8%">
						<col style="width: 15%"> 
						<col style="width: 8%">
		...
		<div id="divIncomeHead" class="dynamicTableContainer" style="height: 400px;">
			<table style="width: 100%; border: #0F0; table-layout: fixed;" class="transTablex col15-center">
				<tr bgcolor="#c0c0c0">
				 	<td width="10%" >Folio No </td>  
					<td width="8%" >Doc Date </td>
					<td width="8%" >Doc No </td>
					<td width="6.5%" >Perticulars</td>
					<td width="6.5%">Quantity</td>
					<td width="6.5%">Amount</td>
					<td width="7.5%">User Name</td>
					<td width="10%">Date Edited </td>
				</tr>
			</table>
		
			<div style="background-color: #fbfafa; border: 1px solid #ccc; display: block; height: 330px; margin: auto; overflow-x: hidden; overflow-y: scroll; width: 100%;">
				<table id="tblIncomeHeadDet" style="width: 100%; border: #0F0; table-layout: fixed;overflow: scroll;display:flex" class="transTablex col15-center">
			...
			<!--  		<colgroup><col style="width:15%">
						...
						<col style="width: 15%">
						<col style="width: 15%">
						<col style="width: 25%;/* padding-right: 7%; */">
						<col style="width: 20%">
					</colgroup>
				-->	
			...
			<table class="transTable" style="width:100%;height:30px; ">
			 ...
			 -->
			           ...
			           <td><input id="btnRefund" type="button"
						class="btn btn-primary center-block" value="Refund"  onclick="funOnClickRefund('divRefund')" style="background-size: 140px 24px; width: 150px; color:#000;" /></td>	
		...
		<div style=" border: 1px solid #ccc; display: block; height: 36px; margin: auto;  width: 94%;">
			<table class="transTable" style="width:100%;height:30px; ">
				...
				<td><input id="btnIncomeHeadflash" type="button"
						class="btn btn-primary center-block" value="Income Head Flash"  onclick="funOnClckIncomeHeadBtn('divIncomeHead')" style="background-size: 140px 24px; width: 150px; color:#000;" /></td>	
			</tr>
				</table>
				</div>		
