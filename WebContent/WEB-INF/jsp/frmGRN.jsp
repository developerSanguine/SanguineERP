...
// 			  }
		...
		function funCalDiscountAmt2()
		{
			...
			<%-- var  Percent='<%=session.getAttribute("Percentage").toString()%>'; --%>
			
				
		   var costRM =$("#txtCostRM").val();
		   var DiscountOn=$("#txtDiscountOn").val();
		   var Percentage=$("#txtPercentage").val();
			
		   
		   if(DiscountOn=='MRP')
				{
				var subtotal=$("#txtMRP").val()*$("#txtQuantity").val();

				var discountAmt=parseFloat(Percentage)*(parseFloat(subtotal)/100);
				$("#txtDiscount").val(parseFloat(discountAmt).toFixed(maxAmountDecimalPlaceLimit));
				}
			else if(DiscountOn=='Purchase')
				{
				
				var subtotal=$("#txtCostRM").val()*$("#txtQuantity").val();
				...
				{
				  $('#txtDiscount').val("0.00").attr('readonly', true);
				}
...
//Calculating Discount
		...
		**/
		...
		var searchUrl1=getContextPath()+"/getProductPrice.html?suppCode="+suppCode+"&prodCode="+code;
			$.ajax({
				    ...
				    success: function(response)
				    {
				    	$("#txtCostRM").val(response[0][1]/currValue);
					},
		    		error: function(jqXHR, exception) {
			            if (jqXHR.status === 0) {
			                alert('Not connect.n Verify Network.');
	...
	function funGetCostRM(code)
	     {
	    	 var suppCode =$("#txtSuppCode").val();
	     	 var GRNDate =$("#txtGRNDate").val();
	    	
	     	var searchUrl = "";
	    	 searchUrl = getContextPath()+ "/loadCostOfRateContract.html?prodCode=" + code+"&suppCode="+suppCode+"&GRNDate="+GRNDate;  
	    	 ...
	    	 $.ajax({
					...
					success : function(response) {
						...
						$.each(response, function(i,item)
						     ...
						     $("#txtCostRM").val(item[0]);
						     $("#txtDiscountOn").val(item[1]);
						     $("#txtPercentage").val(item[2]);
...
//											    		}
			...
			strIssueLocCode,strIssueLocName,code,rework,gTaxAmount,strExpiry,strStkble,strMISCode)
	{
		    ...
		    var rowCount = table.rows.length;
		    var row = table.insertRow(rowCount);	
		    var DiscountAmount= $('#txtDiscount');
		    rowCount=listRow;
		  ...
		  //  row.insertCell(11).innerHTML= "<input name=\"listGRNDtl["+(rowCount)+"].dblDiscount\" type=\"text\"  required = \"required\" style=\"text-align: right;width:100%;border:1px solid #a2a2a2;padding:1px;\" class=\"decimal-places-amt inputText-Auto txtDisc\" size=\"6%\" id=\"txtDiscount."+(rowCount)+"\" value='"+disc+"' onblur=\"funUpdatePrice(this);\">";
		    row.insertCell(11).innerHTML= "<input name=\"listGRNDtl["+(rowCount)+"].dblDiscount\" type=\"text\"  required = \"required\" style=\"text-align: right;width:100%;border:1px solid #a2a2a2;padding:1px;\" class=\"decimal-places-amt inputText-Auto txtDisc\" size=\"6%\"  value='"+disc+"' onblur=\"funUpdatePrice(this);\">"; 
			...
			<div id="tab_container">
				...
				<div id="tab1" class="tab_content">
					<div class="row transTable">
						...
						<div class="col-md-4"></div>
						<div class="col-md-2">
							<label>Product Code</label> <input id="txtProdCode" path="strProdCode"
						...
						<div class="col-md-2">
						  ...
						  <div class="col-md-2">
							<label>Quantity Received</label> <input id="txtQuantity" value=""
								type="text" class="decimal-places numberField" onkeypress="funGetKeyCode(event,'Discount')"  onblur="funCalDiscountAmt2();"" 
								></input>
						</div>
						...
						<div class="col-md-2">
							<label>Unit Price</label> <input type="text" id="txtCostRM"
							 ...
							 <input name="strDiscountOn" type="hidden" id="txtDiscountON"
								value="${strDiscountOn}" style="text-align: right;"
								class="decimal-places-amt numberField"
								></input>
					...
					<div>
							 <input type="hidden" id="txtDiscountOn"
								value="0" style="text-align: right;"
								class="decimal-places-amt numberField"
								></input>
						</div>
						...
						<div>
							 <input type="hidden" id="txtPercentage"
								value="0" style="text-align: right;"
								class="decimal-places-amt numberField"
								></input>
						</div>
                     ...
                     <div class="col-md-2">
						...
						<div class="col-md-2">
							<label>Discount Amount</label> <input id="txtDiscount" value="0"
								style="text-align: right;" type="text" onkeypress="funGetKeyCode(event,'QtyRecv')" > </input>
								
								
						</div>
						<div class="col-md-2">
							<label>MRP</label> <input id="txtMRP" 
