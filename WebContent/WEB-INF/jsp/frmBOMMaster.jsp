		        	...
		        	if(response.strBOMCode!="Invalid Code")
		        		{
			                ...
			                {
								count=i;
								funLoadBomData(listdata[i].strChildCode,listdata[i].strProdName,listdata[i].dblQty,listdata[i].strUOM,listdata[i].dblYieldPer);
...
// 			document.recipeForm.submit();
		...
		function funLoadBomData(strChildCode,strProdName,dblQty,strUOM,dblYieldPer)
		{
		    ...
		    row.insertCell(4).innerHTML= "<input class=\"Box\" size=\"7%\" name=\"listBomDtlModel["+(rowCount)+"].dblYieldPer\" id=\"lblUOM."+(rowCount)+"\" value="+dblYieldPer+">";
		...
		function funAddRow() 
		{
			...
			if(childCode==parentCode)
			    ...
			    var yeildper = document.getElementById("txtYieldPer").value;
			    if(yeildper=="")
		    	{
		    	yeildper=100;
		    	}
			    ...
			    row.insertCell(4).innerHTML= "<input type=\"number\" step=\"any\" required = \"required\" style=\"text-align: right;border:1px solid #a2a2a2;padding:1px;\" class=\"inputText-Auto\" name=\"listBomDtlModel["+(rowCount)+"].dblYieldPer\" id=\"txtQty."+(rowCount)+"\" value="+yeildper+">";
			    row.insertCell(5).innerHTML= '<input type="button" value = "Delete" class="deletebutton" onClick="Javacsript:funDeleteRow(this)">';
