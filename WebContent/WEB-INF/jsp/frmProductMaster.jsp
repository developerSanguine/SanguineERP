...
<script type="text/javascript">
	...
	var listRowChildProdTabGrid=0;
	var clickCount =0.0;
	var dblProdStockItemNotInUse=0;
		...
		function funHelp(transactionName)
		{
			...
			if(fieldName=='ChildProdmasterPropwise')
			{
				transactionName='prodmasterPropwise';
			}
		...
		function funSetSubGroupCategory(code)
		{
				 $.ajax({
					        ...
					        success: function(response)
					        {
					        	if(response.strSGCCode=='Invalid SubGroup Category Code')
					        	{
					        	...
					        	{
					        		$("#txtSubGroupCCode").val(code);
							    	$("#txtSubGroupCName").text(response.strSGCName);
			    ...
			    success: function(response)
			    {
			    	...
			    	else
			    	{
			    		if(fieldName=='ChildProdmasterPropwise')
			    		{
					    	...
					    	if(response.strSGCCode !=""){
					    		funSetSubGroupCategory(response.strSGCCode);
					    	}
					    	...
					    	funLoadChildProd(response.listProdChilddtl);
					    	//$("#cmbProdGroupYN").val(response.strProdGroupYN);
					    	dblProdStockItemNotInUse=response.dblProdStockItemNotInUse;
		...
		{    		    
		    ...
		    row.insertCell(0).innerHTML= "<input readonly=\"readonly\" class=\"Box\"  size=\"15%\"  id=\"txtBatchCode."+(rowCount)+"\" value='"+BatchCode+"' onclick=\"funBatchExel(this)\">";
		...
		function funBatchExel(data)
		{
			
			var BatchCode = data.value;
			window.open(getContextPath()+"/rptBatchReport.html?BatchCode="+BatchCode+"");
		}
		...
		$(function()
				{
					...
					$('#txtSubGroupCCode').keydown(function(e){
						 if (e.which == 9){
		...
		function funCallValidation(actionName,object) {
		if( $('#txtChildProdCode').val().trim()=="")
		{
			alert("Please Select Child Code");
		...
		function funCallFormAction(actionName,object) 
		{
			...
			{
				alert("Please Select SubGroup Category Code");
				//$('#txtSubGroupCode').focus();
				return false;
			}
		/*	if( $('#txtChildProdCode').val().trim()=="")
    ...
    function funLoadChildProd(listChildProdDtl){
    	$.each(listChildProdDtl, function(i,item)
    	    	{
    		funfillChildProdGrid(listChildProdDtl[i].strProdChildCode,
    	    				listChildProdDtl[i].strProdChildName,
    	    				listChildProdDtl[i].dblChildProdCF,
    	    				listChildProdDtl[i].dblGroupProdCF,
    	    				listChildProdDtl[i].strProdGroupYN
    	    				);
    	    	});
    }

    function funfillChildProdGrid(childProdCode,childProdName,childCF,prodGroupCF,prodGroupYN)
	{
		
		 	var table = document.getElementById("tblProdGroup");
		    var rowCount = table.rows.length;
		    var row = table.insertRow(rowCount);
		    rowCount=listRowChildProdTabGrid;
		    
			row.insertCell(0).innerHTML= "<input readonly=\"readonly\" class=\"Box \" size=\"15%\" name=\"listProdChilddtl["+(rowCount)+"].strProdChildCode\" id=\"childProdCode."+(rowCount)+"\" value='"+childProdCode+"' >";
		    row.insertCell(1).innerHTML= "<input readonly=\"readonly\" class=\"Box \" size=\"28%\"  id=\"strCName."+(rowCount)+"\" value='"+childProdName+"' >";
		    row.insertCell(2).innerHTML= "<input type=\"text\"  size=\"20%\" required = \"required\" style=\"text-align: right;border:1px solid #a2a2a2;width:97%;\"  name=\"listProdChilddtl["+(rowCount)+"].dblChildProdCF\" id=\"childCF."+(rowCount)+"\" value='"+childCF+"' >";
		    row.insertCell(3).innerHTML= "<input type=\"text\"  size=\"20%\" required = \"required\" style=\"text-align: right;width:75%;border:1px solid #a2a2a2;\" name=\"listProdChilddtl["+(rowCount)+"].dblGroupProdCF\" id=\"prodGroupCF."+(rowCount)+"\" value='"+prodGroupCF+"' >";
		    row.insertCell(4).innerHTML= "<input type=\"text\"  size=\"20%\" required = \"required\" style=\"text-align: right;width:75%;border:1px solid #a2a2a2;\" name=\"listProdChilddtl["+(rowCount)+"].strProdGroupYN\" id=\"prodGroupYN."+(rowCount)+"\" value='"+prodGroupYN+"' >";
			row.insertCell(5).innerHTML= '<input type="button" class="deletebutton" value = "Delete" onClick="funDeleteRowForChildProduct(this)">';
	
		    listRowChildProdTabGrid++;
	}
    
    function funChildProdField(){
    	var childProdCode=$('#txtChildProdCode').val();
		var childProdName=$('#txtChildProdName').val();
		var childCF=$('#txtChildCF').val();
		var prodGroupCF=$('#txtProdGroupCF').val();
		
	//	var prodGroupYN=$('#cmbProdGroupYN').val();
	//	var prodGroupYN=document.getElementById("strProdGroupYN");
		
		
			if ($("#cmbProdGroupYN").val() == "Y")
			{
				var prodGroupYN = "Y";
			}
			else 
		...
		funfillChildProdGrid(childProdCode,childProdName,childCF,prodGroupCF,prodGroupYN);
	     	  	
    }
    
    function funDeleteRowForChildProduct(obj)
	{
	    var index = obj.parentNode.parentNode.rowIndex;
	    var table = document.getElementById("tblProdGroup");
	    table.deleteRow(index);
	}
    function funShowStockOfProduct(){
    	
    	if(dblProdStockItemNotInUse > 0   && $("#chkNotInUse").prop('checked')){
    		alert("Product Stock : "+ dblProdStockItemNotInUse);
    	}
...
<div class="container">
	...
	<!--  <div class="col-md-12"  align="right" style="margin-right:270px">
			 ...
			 <div class="col-md-3"><label >Name </label>
				 <s:input type="text" id="txtProdName" cssStyle="width:100%;"  name="prodName" path="strProdName"/>
			 </div>
		    ...
		    <div class="col-md-4"><label id="txtSubGroupName" style="background-color:#dcdada94; width:65%; height: 42%; margin: 27px 0px;"></label></div>
			 ...
			 <div class="col-md-2"><label >Sub-Group Category Code</label> 
				    <s:input id="txtSubGroupCCode" name="subGroupCode" path="strSGCCode" ondblclick="funHelp('subgroupcategory')" autocomplete="off" cssClass="searchTextBox"/>
			 </div>
		
		    <div class="col-md-4"><label id="txtSubGroupCName" style="background-color:#dcdada94; width:65%; height: 42%; margin: 27px 0px;"></label></div>
		...
		<div class="container masterTable">
			<div class="row">				 
			   <div class="col-md-2"><label>Item Not In Use</label><br>
					<s:checkbox id="chkNotInUse" name="notInUse" path="strNotInUse" value="" onclick="funShowStockOfProduct()"/>
				</div>
...
<!-- 										onclick="return funAddCharRow()" class="smallButton"></input></td> -->
	...
	<div id="tab8" class="tab_content" style="height:450px">
		<br> <br>
	<div class="container masterTable">
		<div class="row">
			...
			<br><br>
			 <div class="col-md-2"><label >Child Code </label>
				  <s:input  id="txtChildProdCode" name="childprodCode"  path="" value="" ondblclick="funHelp('ChildProdmasterPropwise')" cssClass="searchTextBox"/>
			</div>	
			
				    		    
			 <div class="col-md-3"><label >Child Name </label>
				 <s:input type="text" id="txtChildProdName" path="" cssStyle="width:80%;text-transform: uppercase;"  name="childProdName"/>
			 </div>
			
			<div class="col-md-2"><label>Child CF</label>
				<s:input id="txtChildCF" path="" name="recipeConversionRatio" style="width:50%;text-align:right;"  value="1.0" cssClass="decimal-places-amt numberField" />
			</div>
			<div class="col-md-2"><label>Prod Group CF</label>
				<s:input id="txtProdGroupCF" path="" name="recipeConversionRatio" style="width:50%;text-align:right;"  value="1.0" cssClass="decimal-places-amt numberField" />
			</div>
	      <div class="col-md-3">
	      <label>Prod Group Y/N </label>
	     <s:select id="cmbProdGroupYN" path="strProdGroupYN"  name="cmbProdGroupYN" style="width:auto;">
		 ...
		 <div class="col-md-2">	      <br>  			
			 <input id="btnAddChildProduct" type="button" value="Add Child Product" onclick="funChildProdField()"  class="btn btn-primary center-block" 
			  style="height:30px;width :100%" ></input>
		</div>
         		
        		
			<table  style="margin-left: -15.4%;margin-top: 8%;height:20px;border:#0F0;width:65%;font-size:11px;font-weight: bold;">	
		
				    <tr bgcolor="#c0c0c0" >
							<td width="7%">Child Code</td><!--  COl1   -->
							<td width="12%">Child Name</td><!--  COl2   -->
							<td width="7%">Child CF</td><!--  COl3   -->
							<td width="10%">Prod Group CF</td><!--  COl4   -->	
							<td width="4%">Prod Group Y/N</td><!--  COl5   -->								
							<td width="1%">Delete</td><!--  COl6   -->
		
					</tr>
			</table>
						
				<div style="margin-left:1.2%;background-color:#fafbfb; border: 1px solid #ccc; display: block; height: 150px;overflow-x: hidden; overflow-y: scroll; width:65%;">
					   <table id="tblProdGroup" style="width:100%;border: #0F0;table-layout:fixed;overflow:scroll" class="transTablex ">
							<tbody>    
