...
$(document).ready(function(){
	//window.location.href=getContextPath()+"/loadPendingRequisition.html";
		...
		<%if(session.getAttribute("selectedModuleName").toString().equalsIgnoreCase("6-WebCRM")){%>
   		funGetCRMNotification();
		<%}%>
...
<%if(session.getAttribute("selectedModuleName").toString().equalsIgnoreCase("6-WebCRM")){%>
NotificationTimeinterval=parseInt(NotificationTimeinterval)*60000;
setInterval(function(){funGetCRMNotification()},NotificationTimeinterval);
...
//Notification for pending sales order
 ...
 function funGetCRMNotification()
{
	var searchUrl=getContextPath()+"/getCRMNotification.html";
	$.ajax({
      type: "GET",
      url: searchUrl,
      dataType: "json",
      success: function(response)
      {
      	...
      	$.each(response, function(i,item)
      	        {
      				count=i;
      				funfillCRMNotificationRow(response[i][0],response[i][1],response[i][2],response[i][3],response[i][4]);
      	        });
      	if(response.length>0)
      		{
      			NotificationCount=count+1;	
...
function funfillCRMNotificationRow(strSOCode,strPName,dtlFulmtDate,dblTotal,formName) 
{
    var table = document.getElementById("tblNotify");
    var rowCount = table.rows.length;
    var row = table.insertRow(rowCount);
    ...
    //row.insertCell(0).innerHTML= "<label>"+strReqCode+"</label>";
    row.insertCell(0).innerHTML= "<input class=\"Box\" readonly = \"readonly\" size=\"24%\" onClick=\"funClick('"+formName+"','"+strSOCode+"','');\" value='"+strSOCode+"'>";
 	row.insertCell(1).innerHTML= "<input class=\"Box\" readonly = \"readonly\" size=\"54%\" onClick=\"funClick('"+formName+"','"+strPName+"','');\" value='"+strPName+"'>";
 	row.insertCell(2).innerHTML= "<input class=\"Box\" readonly = \"readonly\" size=\"24%\" onClick=\"funClick('"+formName+"','"+dtlFulmtDate+"','');\" value='"+dtlFulmtDate+"'>";
 	row.insertCell(3).innerHTML= "<input class=\"Box\" readonly = \"readonly\" size=\"24%\" onClick=\"funClick('"+formName+"','"+dblTotal+"','');\" value='"+dblTotal+"'>";
...
function funRemoveNotification()
{
	...
	function funClick(formName,docCode,strLocOn,locationby)
	{
		switch (formName)
		{
			...
			case "frmSalesOrder" :
				window.open("frmBulkInvoice.html");
				break;
				
				
			case "Expiry" :
				window.location.href=getContextPath()+"/ExportExpiryFlashReport.html?param1=ExpiryNotificationClick"; 
		}
...
function funHelpWindow(formName)
{
	...
	switch (formName)
	{
		...
		window.open("https://youtu.be/LJeSX-avzGM", "myhelp", "scrollbars=1,width=500,height=350");
		//window.open("https://youtu.be/8vl_eSatRDM", "myhelp", "scrollbars=1,width=500,height=350");
		
		break;	
	
	
		case  "frmMaterialReq" :
			window.open("https://youtu.be/FOgCFt7k2gg", "myhelp", "scrollbars=1,width=500,height=350");
		...
		case "frmPhysicalStkPosting":
			window.open("https://youtu.be/aO_4LEO5xVY", "myhelp", "scrollbars=1,width=500,height=350");
			break;
			
		case "frmPurchaseIndent":
			window.open("https://youtu.be/zNCg0z6Velk", "myhelp", "scrollbars=1,width=500,height=350");
			break;
				
		case "frmPurchaseOrder":
			window.open("https://youtu.be/xebE91pLd-Q", "myhelp", "scrollbars=1,width=500,height=350");
			break;
			
		case "frmProductMaster":
			window.open("https://youtu.be/CQsIIGb6nBc", "myhelp", "scrollbars=1,width=500,height=350");
			break; 
			
		case "frmBOMMaster":
			window.open("https://youtu.be/375TyC1zI1I", "myhelp", "scrollbars=1,width=500,height=350");
			break; 
			
		case "frmSupplierMaster":
			window.open("https://youtu.be/lh20brCD22M", "myhelp", "scrollbars=1,width=500,height=350");
			break;
			
		case "frmLocationMaster":
			window.open("https://youtu.be/Gb--jF1v10Q", "myhelp", "scrollbars=1,width=500,height=350");
			break;
			
		case "frmStockFlash":
			window.open("https://youtu.be/LT0BRpT5Vy8", "myhelp", "scrollbars=1,width=500,height=350");
			break;
			
		case "frmStockLedger":
			window.open("https://youtu.be/Nmkj460ahYA", "myhelp", "scrollbars=1,width=500,height=350");
			break;
			
		case "frmTallyLinkUp":
			window.open("https://youtu.be/Amrq4oEuIZc", "myhelp", "scrollbars=1,width=500,height=350");
			break;
		...
		case "frmBillPassing":
	 	...
	 	case "frmWebStockHelpGRN":
			window.open("https://youtu.be/NE6y_RGWThs", "myhelp", "scrollbars=1,width=500,height=350");
			break;
		...
		case "frmWebStockHelpRateContract":
			window.open("https://youtu.be/oDlfeGF9crM", "myhelp", "scrollbars=1,width=500,height=350");
			break;
