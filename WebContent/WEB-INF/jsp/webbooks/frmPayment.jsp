...
// Fill Account details and Debtor Details Grid	
	...
	function funFillGRNDtlGrid(resListPaymentGRNDtlBean)
	{
        ...
        {			
			...
			funLoadGRNTable(resListPaymentGRNDtlBean[i].strGRNCode,resListPaymentGRNDtlBean[i].strGRNBIllNo,resListPaymentGRNDtlBean[i].dblGRNAmt,
					resListPaymentGRNDtlBean[i].dteGRNDate,resListPaymentGRNDtlBean[i].dteBillDate,resListPaymentGRNDtlBean[i].dteGRNDueDate,resListPaymentGRNDtlBean[i].strSelected,resListPaymentGRNDtlBean[i].dblPayedAmt);
...
// Function to Validate Header Fields
		...
		$.ajax({
			...
			success : function(response){ 
				$.each(response, function(i, item) {
						...
						funLoadGRNTable(response[i].strGRNCode,response[i].strGRNBIllNo,response[i].dblGRNAmt,response[i].dteGRNDate,response[i].dteBillDate,response[i].dteGRNDueDate,"No","0");				
				});	
	...
	function funLoadGRNTable(strGRNCode,strGRNBIllNo,dblGRNAmt,dteGRNDate,dteBillDate,dteGRNDueDate,isSelected,dblPayedAmt)
	{
		var totalGrnAmt = parseFloat(dblGRNAmt);
		 var table = document.getElementById("tblGRN");
		    ...
		    row.insertCell(7).innerHTML= "<input class=\"text totalGRNPayedCell\"  required = \"required\" style=\"text-align: right;width:98%;\" size=\"9%\" name=\"listPaymentGRNDtl["+(rowCount)+"].dblPayedAmt\" id=\"txtPayedAmt."+(rowCount)+"\" value="+dblPayedAmt+" onblur=\"funUpdateAmount(this);\" >";
...
// 		     $("#txtTotalGRNAmount").val(totalGrnAmt );
	...
	function funGRNChkOnClick()
	{
	    ...
	    $("#txtGRNPayAmount").val(grnAmt.toFixed(maxAmountDecimalPlaceLimit));
