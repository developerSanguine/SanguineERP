...
public class clsPMSPaymentServiceImpl implements clsPMSPaymentService {
	...
	public clsPMSPaymentHdModel funPreparePaymentModel(clsPMSPaymentBean objPaymentBean, String clientCode,  HttpServletRequest request, String userCode) {
		...
		String PMSDate= objPaymentBean.getDtePaymentDate();
		String[] splitDate = PMSDate.split("-");
		String PaymentDate = splitDate[2] + "-" + splitDate[1] + "-" + splitDate[0];
		String RecieptNO= objPaymentBean.getStrReceiptNo();
		...
		if (objPaymentBean.getStrReceiptNo()!=null && objPaymentBean.getStrReceiptNo().isEmpty()) {
			...
			objModel.setDteReceiptDate( objGlobal.funGetDate("yyyy-MM-dd",objPaymentBean.getDtePaymentDate()));
			...
			if (objPaymentBean.getStrAgainst().equals("Reservation")) {
				...
				if (list.size() > 0) {
	 				for (int cnt = 0; cnt < list.size(); cnt++) {
	 					...
	 					if(guestlist!=null && guestlist.size()>0)
						{
	 					...
	 					String sqlguest = " update tblguestmaster a , tblreceipthd b set a.dblClosingBalance='"+amount1+"' "
	 										+ " and b.dteReceiptDate='"+PaymentDate+"' where  a.strGuestCode='"+guestCode+"' and b.strReceiptNo='"+RecieptNO+"' ";
				...
				if (list.size() > 0) {
					for (int cnt = 0; cnt < list.size(); cnt++) {
						...
						String sqlguest=" update tblguestmaster a,tblreceipthd b set a.dblClosingBalance='"+amount2+"'"
									  + " and b.dteReceiptDate='"+PaymentDate+"' where  a.strGuestCode='"+guestCode+"' and b.strReceiptNo='"+RecieptNO+"' ";
	 					objWebPMSUtility.funExecuteUpdate(sqlguest, "sql");
					}
				}
