			...
			@RequestParam("billNo") String billNo,@RequestParam("strSelectBill") String strSelectBill, HttpServletRequest req,HttpServletResponse resp) 
	{
		...
		try 
		{
			...
			String sqlParametersFromBill =  " SELECT a.strFolioNo,e.strRoomDesc,a.strRegistrationNo,a.strReservationNo ,date(b.dteArrivalDate),b.tmeArrivalTime , "
					...
					+ " b.intNoOfAdults,b.intNoOfChild ,a.strBillNo ,IFNULL(d.strGuestCode,'')"
					+ ",a.strGSTNo,a.strCompanyName,b.strCheckInNo,c.strPayee,DATE_FORMAT(a.dteBillDate,'%d-%m-%Y'),h.strPlanDesc"// 21
					+ " FROM tblbillhd a  "
					+ " LEFT OUTER JOIN tblcheckinhd  b ON a.strCheckInNo=b.strCheckInNo "
					+ " LEFT OUTER JOIN tblcheckindtl c ON b.strCheckInNo=c.strCheckInNo "
			...
			if (listOfParametersFromBill.size() > 0) {
				...
				String strBillDate = arr[20].toString();
				...
				if(arr[21]!=null) 
				{
					reportParams.put("Plan", arr[21].toString());
				}
				else
				{
					reportParams.put("Plan", "NA");
				}
				String guestAddr = "";
...
//				String noOfRoom = objPropertySetupModel.getStrRoomLimit();.
				...
				reportParams.put("pDteBillDate", strBillDate);
				...
				if(!(billNames.length()>0))
				{
					sqlBillDtl = "SELECT DATE(b.dteDocDate),b.strDocNo,"
							...
							+ "WHERE a.strBillNo='"+billNo+"' and  b.dblDebitAmt >0 "
							+ "group by b.strPerticulars";
				}
				else if((strSelectBill.length()>0))
				{
					
					String[] temp1 = strSelectBill.split(",");
					billNames="";
					...
					billNames = billNames.substring(0, billNames.length()-1);
					...
					sqlBillDtl = "SELECT DATE(b.dteDocDate),b.strDocNo,"
						...
						//	+ "ON a.strFolioNo=b.strFolioNo AND a.strBillNo=b.strBillNo AND ( b.strPerticulars IN('Package','Room Tariff') or b.strRevenueType='Income Head') "
							+ "ON a.strFolioNo=b.strFolioNo AND a.strBillNo=b.strBillNo "
							+ "WHERE a.strBillNo='"+billNo+"' AND a.strClientCode='"+clientCode+"' and  b.dblDebitAmt >0  and b.strPerticulars in (" + billNames + ") order by b.dblCreditAmt ,b.dteDocDate";
				}
				...
				for (int i = 0; i < billDtlList.size(); i++) {
					...
					if(strSelectBill.contains("Room Tariff"))
						...
						{
				    		reportParams.put("isRoomTariff", 0.0);
							}
						}			
						else
						{
