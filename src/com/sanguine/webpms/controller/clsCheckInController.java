...
public class clsCheckInController {
	...
	public ModelAndView funAddUpdate(@ModelAttribute("command") @Valid clsCheckInBean objBean, BindingResult result, HttpServletRequest req) {
		if (!result.hasErrors()) {
			...
			String CheckinEditNo=objBean.getStrCheckInNo();
			if(null!=CheckinEditNo)
			{
			...
			if(listFolioNo.size()>0)
			{
 				...
 				if(listFolioNoDtl.size()>0)
				{
					...
					{
					/*	clsReservationHdModel objReservationModel = new clsReservationHdModel() ;
					    String sqlroom="delete from tblreservationroomratedtl where strReservationNo='"+objReservationModel.getStrReservationNo()+"' ";							
					...
					//	String sqlFolioPackage=" UPDATE tblfoliodtl a SET a.dblDebitAmt='"+objPkgDtlBean.getDblIncomeHeadAmt()+"' WHERE a.strFolioNo='"+strFolioNo+"' and  a.strPerticulars='Package' ";
						String sqlFolioPackage=" UPDATE tblfoliodtl a SET a.dblDebitAmt='"+objPkgDtlBean.getDblIncomeHeadAmt()+"' WHERE a.strFolioNo='"+strFolioNo+"' ";
        ...
        public void funGeneratePaymentRecipt(@RequestParam("checkInNo") String reciptNo,@RequestParam("cmbAgainst") String strAgainst , HttpServletRequest req, HttpServletResponse resp) {
			try {
				...
				String sql = "SELECT a.strCheckInNo,a.strGuestCode,f.strRoomDesc,"
						...
						+ " IFNULL(e.strAddressLocal,''), IFNULL(e.strCityLocal,''), "
						+ " IFNULL(e.strStateLocal,''), IFNULL(e.strCountryLocal,''), "
						+ " IFNULL(e.intPinCodeLocal,''),IFNULL(e.lngMobileNo,0),d.strComplimentry,DATE_FORMAT(d.dteArrivalDate,'%d-%m-%Y'),DATE_FORMAT(d.dteDepartureDate,'%d-%m-%Y'),e.strAddressLocal,e.strEmailId,  "
						+ " e.strPassportNo,e.dtePassportExpiryDate,e.dtePassportIssueDate,e.strNationality,DATE_FORMAT(d.dteDateCreated,'%d-%m-%Y') "
				...
				if(!arrObjRoomData[15].toString().equalsIgnoreCase(""))
				{
					guestCompanyAddr=arrObjRoomData[15].toString();
				}

				if(!arrObjRoomData[16].toString().equalsIgnoreCase(""))
				{
					guestCompanyAddr=guestCompanyAddr + "," + arrObjRoomData[16].toString();
				}
				else if(!arrObjRoomData[17].toString().equalsIgnoreCase(""))
				{
					guestCompanyAddr=guestCompanyAddr + "," + arrObjRoomData[17].toString();
				}
				else if(!arrObjRoomData[18].toString().equalsIgnoreCase(""))
				{
					guestCompanyAddr=guestCompanyAddr + "," + arrObjRoomData[18].toString();
				}
				else if(!arrObjRoomData[19].toString().equalsIgnoreCase("0"))
				{
					guestCompanyAddr=guestCompanyAddr + "," + arrObjRoomData[19].toString();
				}
				else
				{
				...
				if(clientCode.equalsIgnoreCase("378.001"))
				{
					...
					reportParams.put("dteChckInGenDate",arrObjRoomData[31].toString());
				...
				else
				{
					String sqlReservation ="SELECT a.strCheckInNo,a.strGuestCode,"
							...
							+ " IFNULL(e.strAddressLocal,''), IFNULL(e.strCityLocal,'') , "
							+ " IFNULL(e.strStateLocal,''), IFNULL(e.strCountryLocal,'') , "
							+ " IFNULL(e.intPinCodeLocal,''), IFNULL(e.lngMobileNo,0) ,d.strComplimentry,DATE_FORMAT(d.dteArrivalDate,'%d-%m-%Y'),DATE_FORMAT(d.dteDepartureDate,'%d-%m-%Y') ,h.dblRoomRate,e.strAddressLocal,e.strEmailId, "
							+ " e.strPassportNo,e.dtePassportExpiryDate,e.dtePassportIssueDate,e.strNationality,DATE_FORMAT(d.dteDateCreated,'%d-%m-%Y') "
				...
				if (jp != null) {
					...
					resp.setHeader("Content-Disposition", "inline;filename=CheckInSlip.pdf");
