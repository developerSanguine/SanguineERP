...
public class clsPMSSalesFlashController {
	...
	public @ResponseBody List funLoadSettlementWiseDtl(
			HttpServletRequest request) {
		...
		String sql = "select c.strSettlementDesc,sum(b.dblSettlementAmt) "
				...
				+ " and a.strClientCode=b.strClientCode and b.strClientCode='"
				+ strClientCode + "' AND c.strClientCode='" + strClientCode
				+ "'" + " group by b.strSettlementCode;";

		List listSettlementDtl = objGlobalService.funGetListModuleWise(sql,
				"sql");
		if (!listSettlementDtl.isEmpty()) {
			for (int i = 0; i < listSettlementDtl.size(); i++) {
				...
				dblTotalValue = new BigDecimal(Double.parseDouble(arr2[1]
						.toString())).add(dblTotalValue);
			}
		}
	...
	public @ResponseBody List funLoadRevenueHeadWiseDtl(
			HttpServletRequest request) {
		String fromDate = request.getParameter("frmDte").toString();
		String strClientCode = request.getSession().getAttribute("clientCode")
				.toString();
		...
		String sql = " SELECT 'Bill Revenue', b.strRevenueType AS strRevenueType, SUM(b.dblDebitAmt) AS Amount"
				...
				+ " WHERE a.strBillNo=b.strBillNo AND DATE(a.dteBillDate) BETWEEN  '"
				+ fromDte
				+ "'  AND '"
				+ toDte
				+ "'  "
				+ " AND a.strClientCode='"
				+ strClientCode
				+ "' AND b.strClientCode='378.001' GROUP BY b.strRevenueType"
				+ " UNION ALL"
		...
		if (!listRevenueDtl.isEmpty()) {
			for (int i = 0; i < listRevenueDtl.size(); i++) {
				...
				clsPMSSalesFlashBean objBean = new clsPMSSalesFlashBean();
				objBean.setStrRevenueType(arr2[0].toString() + "#"
						+ arr2[1].toString());
				objBean.setDblAmount(Double.parseDouble(arr2[2].toString()));
				dblTotalValue = new BigDecimal(Double.parseDouble(arr2[2]
						.toString())).add(dblTotalValue);
	...
	public @ResponseBody List funTaxWiseDtl(HttpServletRequest request) {
		...
		if (!listTaxDtl.isEmpty()) {
			for (int i = 0; i < listTaxDtl.size(); i++) {
				...
				dblTotalValue = new BigDecimal(Double.parseDouble(arr2[1]
						.toString())).add(dblTotalValue);
				dblTaxTotalValue = new BigDecimal(arr2[2].toString())
						.add(dblTaxTotalValue);
			}
		}
	...
	public @ResponseBody List funExpectedArrWiseDtl(HttpServletRequest request) {
		...
		String sql = "SELECT a.strReservationNo, DATE_FORMAT(a.dteDateCreated,'%d-%m-%Y'), "
				+ " ifnull(CONCAT(e.strFirstName,' ',e.strMiddleName,' ',e.strLastName),''),g.strRoomTypeDesc,f.strRoomDesc,DATE_FORMAT(a.dteArrivalDate,'%d-%m-%Y'), DATE_FORMAT(a.dteDepartureDate,'%d-%m-%Y'),"
				+ " IFNULL(j.DepositAmt,0), IFNULL(i.dblReceiptAmt,0) "
				...
				+ " LEFT OUTER JOIN tblreceipthd i ON i.strReservationNo=a.strReservationNo"
				+ " LEFT OUTER JOIN (select b.strCustomerCode GuestCode, sum(a.dblReceiptAmt) DepositAmt  from tblreceipthd a, tblreceiptdtl b where "
				+ " a.strReceiptNo = b.strReceiptNo and a.strAgainst = 'Deposit' "
				+ " group by b.strCustomerCode) j on a.strGuestcode = j.GuestCode"
				+ " WHERE  "
				+ "  a.strClientCode='"
				+ strClientCode
				+ "'  AND a.strReservationNo NOT IN ( SELECT strReservationNo FROM tblcheckinhd) AND a.strCancelReservation = 'N' "
				+ "  AND a.dteArrivalDate >= '"
				+ fromDte
				+ "' AND a.dteArrivalDate <= '" + toDte + "'";
		...
		if (!listArrivalDtl.isEmpty()) {
			for (int i = 0; i < listArrivalDtl.size(); i++) {
				...
				objBean.setStrRoomDesc(arr2[4].toString());
				objBean.setDteArrivalDate(arr2[5].toString());
				objBean.setDteDepartureDate(arr2[6].toString());
				objBean.setDblDepositAmount(arr2[7].toString());
				objBean.setDblReceiptAmt(arr2[8].toString());
				listofExpectedArrDtl.add(objBean);
				dblTotalAmt=new BigDecimal(Double.parseDouble(arr2[7]
						.toString())).add(dblTotalAmt);

				dblTotalValue = new BigDecimal(Double.parseDouble(arr2[8]
						.toString())).add(dblTotalValue);

			}
		}
		listofExpectedArrTotal.add(listofExpectedArrDtl);
		listofExpectedArrTotal.add(dblTotalAmt);
	...
	public @ResponseBody List funExpectedDeptWiseDtl(HttpServletRequest request) {
		...
		String sql = "SELECT a.strCheckInNo,a.strType, DATE_FORMAT(a.dteDepartureDate,'%d-%m-%Y') ,c.strRoomDesc,c.strRoomTypeDesc,"
				...
				+ strClientCode
				+ "'"
				+ " AND DATE(a.dteDepartureDate) BETWEEN '"
				+ fromDte
				+ "' AND '"
				+ toDte
				+ "'"
		...
		List listExpectedDeptDtl = objGlobalService.funGetListModuleWise(sql,
				"sql");
	...
	public @ResponseBody List funCheckInDtl(HttpServletRequest request) {
		...
		String sql = " SELECT a.strCheckInNo,a.strType, DATE_FORMAT(a.dteArrivalDate,'%d-%m-%Y'),c.strRoomDesc,c.strRoomTypeDesc, "
				...
				+ " AND d.strClientCode='"
				+ strClientCode
				+ "' AND DATE(a.dteCheckInDate) BETWEEN '"
				+ fromDte
				+ "' AND '" + toDte + "' ; ";
	...
	public @ResponseBody List funCheckOutDtl(HttpServletRequest request) {
		...
		String sql = "SELECT   a.strCheckInNo,a.strType, DATE_FORMAT(e.dteBillDate,'%d-%m-%Y'),c.strRoomDesc,c.strRoomTypeDesc,"
				...
				+ " AND e.strClientCode='"
				+ strClientCode
				+ "' AND DATE(e.dteBillDate) BETWEEN '"
				+ fromDte
				+ "' AND '"
				+ toDte + "';";

		List listCheckOutDtl = objGlobalService
				.funGetListModuleWise(sql, "sql");
		if (!listCheckOutDtl.isEmpty()) {
			for (int i = 0; i < listCheckOutDtl.size(); i++) {
				...
				dblTotalValue = new BigDecimal(Double.parseDouble(arr2[6]
						.toString())).add(dblTotalValue);
			}
		}
	...
	public @ResponseBody List funCancelationDtl(HttpServletRequest request) {
		...
		String sql = "SELECT a.strReservationNo, CONCAT(c.strFirstName,' ',c.strMiddleName,' ',c.strLastName) AS strGuestName, e.strBookingTypeDesc,h.strRoomTypeDesc,DATE_FORMAT(b.dteReservationDate,'%d-%m-%Y') AS dteReservationDate,DATE_FORMAT(a.dteCancelDate,'%d-%m-%Y') AS dteCancelDate,f.strRoomDesc, g.strReasonDesc, a.strRemarks "
				...
				+ " WHERE DATE(a.dteCancelDate) BETWEEN '"
				+ fromDte
				+ "' AND '"
				+ toDte
				+ "' AND a.strReservationNo=b.strReservationNo AND b.strCancelReservation='Y' AND b.strReservationNo=d.strReservationNo "
		...
		List listCancelationDtl = objGlobalService.funGetListModuleWise(sql,
				"sql");
	...
	public @ResponseBody List funNoShowDtl(HttpServletRequest request) {
		...
		String[] arr1 = toDate.split("-");
		String toDte = arr1[2] + "-" + arr1[1] + "-" + arr1[0];
		String PMSDate = objGlobal.funGetDate("yyyy-MM-dd", request
				.getSession().getAttribute("PMSDate").toString());
		...
		String sql = " SELECT CONCAT(c.strFirstName,' ',c.strMiddleName,' ',c.strLastName),"
				...
				+ " WHERE a.strReservationNo=d.strReservationNo AND d.strGuestCode=c.strGuestCode AND d.strClientCode='"
				+ strClientCode
				+ "' AND "
				+ " DATE(a.dteArrivalDate) BETWEEN '"
				+ fromDte
				+ "' AND '"
				+ toDte
				+ "' AND DATE(a.dteArrivalDate) <= '"
				+ PMSDate
				+ "' "
	...
	public @ResponseBody List funVoidBillDtl(HttpServletRequest request) {
		...
		String sql = "SELECT a.strBillNo, DATE_FORMAT(a.dteBillDate,'%d-%m-%Y'),CONCAT(e.strGuestPrefix,\" \",e.strFirstName,\" \",e.strLastName) AS gName,d.strRoomDesc,b.strPerticulars, "
				...
				+ " FROM tblvoidbillhd a inner join tblvoidbilldtl b on a.strBillNo=b.strBillNo AND a.strClientCode='"
				+ strClientCode
				+ "' AND b.strClientCode='"
				+ strClientCode
				+ "'"
				+ " left outer join tblcheckindtl c on a.strCheckInNo=c.strCheckInNo AND c.strClientCode='"
				+ strClientCode
				+ "'"
				+ " left outer join tblroom d on a.strRoomNo=d.strRoomCode AND d.strClientCode='"
				+ strClientCode
				+ "'"
				+ " left outer join tblguestmaster e on c.strGuestCode=e.strGuestCode  AND e.strClientCode='"
				+ strClientCode
				+ "'"
				+ " where c.strPayee='Y' "
				+ " AND DATE(a.dteBillDate) BETWEEN '"
	...
	@RequestMapping(value = "/loadRefundForSalesFlash", method = RequestMethod.GET)
	public @ResponseBody List funRefundDtl(HttpServletRequest request) {
		...
		String sql = "select a.strReceiptNo,IFNULL(DATE_FORMAT((a.dteReceiptDate),'%d-%m-%Y'),''), "
				+ " CONCAT(e.strFirstName,' ',e.strMiddleName,' ',e.strLastName), a.strAgainst, a.strFolioNo , g.strSettlementDesc,"
				+ " a.dblReceiptAmt, f.strRemarks"
				+ " FROM tblreceipthd a, tblcheckinhd b,tblcheckindtl c, tblroom d, tblguestmaster e, tblreceiptdtl f, tblsettlementmaster g "
				+ " where a.strCheckInNo = b.strCheckInNo and b.strCheckInNo=c.strCheckInNo and c.strRoomNo = d.strRoomCode "
				+ " and c.strGuestCode = e.strGuestCode and a.strReceiptNo = f.strReceiptNo and f.strSettlementCode = g.strSettlementCode "
				+ " and a.strAgainst = 'Folio-No' and DATE(a.dteReceiptDate) BETWEEN  '"
				+ fromDte
				+ "' AND '"
				+ toDte
				+ "' and a.strType LIKE '%Refund%' "
				+ " UNION "
				+ "select a.strReceiptNo,IFNULL(DATE_FORMAT((a.dteReceiptDate),'%d-%m-%Y'),''), "
				+ " CONCAT(e.strFirstName,' ',e.strMiddleName,' ',e.strLastName), a.strAgainst,a.strReservationNo, g.strSettlementDesc, "
				+ " a.dblReceiptAmt, f.strRemarks "
				+ " from tblreceipthd a, tblroom d, tblguestmaster e, tblreceiptdtl f, "
				+ " tblsettlementmaster g,tblreservationhd m, tblreservationdtl n "
				+ " where a.strReservationNo = m.strReservationNo  and m.strReservationNo=n.strReservationNo "
				+ " and n.strRoomNo = d.strRoomCode and m.strGuestcode= e.strGuestCode and a.strReceiptNo = f.strReceiptNo "
				+ " and f.strSettlementCode = g.strSettlementCode and a.strAgainst = 'Reservation' "
				+ " and DATE(a.dteReceiptDate) BETWEEN  '"
				+ fromDte
				+ "' AND '"
				+ toDte
				...
				+ " UNION "
				+ "SELECT a.strReceiptNo,IFNULL(DATE_FORMAT((a.dteReceiptDate),'%d-%m-%Y'),''), "
				+ " CONCAT(e.strFirstName,' ',e.strMiddleName,' ',e.strLastName), a.strAgainst,a.strBillNo ,g.strSettlementDesc,"
				+ " a.dblReceiptAmt, f.strRemarks from tblreceipthd a, tblcheckinhd b, tblroom d, tblguestmaster e, tblreceiptdtl f, "
				+ " tblsettlementmaster g, tblbillhd h, tblcheckindtl i where a.strCheckInNo = b.strCheckInNo and b.strCheckInNo = i.strCheckInNo "
				+ " and i.strRoomNo = d.strRoomCode and i.strGuestCode = e.strGuestCode and a.strReceiptNo = f.strReceiptNo "
				+ " and f.strSettlementCode = g.strSettlementCode and b.strCheckInNo = h.strCheckInNo and a.strAgainst = 'Bill' "
				+ " and DATE(a.dteReceiptDate) BETWEEN  '"
				+ fromDte
				+ "' AND '"
				+ toDte
				...
				+ " UNION "
				+ "SELECT a.strReceiptNo,IFNULL(DATE_FORMAT((a.dteReceiptDate),'%d-%m-%Y'),''), "
				+ " CONCAT(e.strFirstName,' ',e.strMiddleName,' ',e.strLastName), a.strAgainst,a.strCheckInNo, g.strSettlementDesc,"
				+ " a.dblReceiptAmt, f.strRemarks from tblreceipthd a, tblcheckinhd b, tblroom d, tblguestmaster e, tblreceiptdtl f,"
				+ " tblsettlementmaster g, tblbillhd h, tblcheckindtl i where a.strCheckInNo = b.strCheckInNo "
				+ " and b.strCheckInNo = i.strCheckInNo and i.strRoomNo = d.strRoomCode and i.strGuestCode = e.strGuestCode "
				+ " and a.strReceiptNo = f.strReceiptNo and f.strSettlementCode = g.strSettlementCode and b.strCheckInNo = h.strCheckInNo and a.strAgainst = 'Check-In'"
				+ " and DATE(a.dteReceiptDate) BETWEEN  '"
				+ fromDte
				+ "' AND '"
				+ toDte
				+ "' and a.strType LIKE '%Refund%' "
				+ " UNION "
				+ "select a.strReceiptNo,IFNULL(DATE_FORMAT((a.dteReceiptDate),'%d-%m-%Y'),''), CONCAT(e.strFirstName,' ',e.strMiddleName,' ',e.strLastName), a.strAgainst, f.strCustomerCode, g.strSettlementDesc,"
				+ " a.dblReceiptAmt, f.strRemarks from tblreceipthd a, tblguestmaster e, tblreceiptdtl f,tblsettlementmaster g "
				+ " where a.strReceiptNo = f.strReceiptNo and f.strCustomerCode = e.strGuestCode and f.strSettlementCode = g.strSettlementCode	and a.strAgainst = 'Deposit'"
				+ " and DATE(a.dteReceiptDate) BETWEEN  '"
				+ fromDte
				+ "' AND '" + toDte + "' and a.strType LIKE '%Refund%' ";
		...
		if (!listVoidBill.isEmpty()) {
			for (int i = 0; i < listVoidBill.size(); i++) {
				...
				objBean.setDteReceiptDate(arr2[1].toString());
				objBean.setStrGuestName(arr2[2].toString());
				objBean.setStrType(arr2[3].toString());
				objBean.setStrDocCode(arr2[4].toString());
				objBean.setStrSettlement(arr2[5].toString());
				objBean.setDblAmount(Double.parseDouble(arr2[6].toString()));
				objBean.setStrRemark(arr2[7].toString());
	...
	public @ResponseBody List funPaymentDtl(HttpServletRequest request) {
		...
		/*
		 * String sql =
		 * "SELECT a.strReceiptNo, DATE(a.dteReceiptDate), CONCAT(d.strFirstName,' ',d.strMiddleName,' ',d.strLastName),"
		 * + " a.strAgainst,e.strSettlementDesc,a.dblReceiptAmt ,b.strRemarks" +
		 ...
		 * +"' AND b.strClientCode='"+strClientCode+"' " +
		 * " AND d.strClientCode='"
		 * +strClientCode+"' AND e.strClientCode='"+strClientCode+"'" +
		 * " AND DATE(a.dteReceiptDate) BETWEEN '"+fromDte+"' AND '"+toDte+"' ";
		 */

		/*
		 * String sql =
		 * "SELECT a.strReceiptNo,IFNULL(DATE_FORMAT((f.dteCheckInDate),'%d-%m-%Y'),''), IFNULL(DATE_FORMAT((f.dteDepartureDate),'%d-%m-%Y'),''),"
		 * +
		 * "   CONCAT(IFNULL(h.strRoomDesc,''),IFNULL(j.strRoomDesc,'')),IFNULL(l.strRoomDesc,''),DATE(a.dteReceiptDate), CONCAT(d.strFirstName,' ',d.strMiddleName,' ',d.strLastName), a.strAgainst,e.strSettlementDesc,a.dblReceiptAmt,"
		 * +
		 * "   b.strRemarks FROM tblreceipthd a LEFT JOIN tblcheckinhd f ON a.strCheckInNo = f.strCheckInNo  "
		 * + "   LEFT JOIN tblbillhd g ON a.strBillNo = g.strBillNo " +
		 * "   LEFT JOIN tblroom h ON g.strRoomNo = h.strRoomCode " +
		 * "   LEFT JOIN tblfoliohd i ON a.strFolioNo = i.strFolioNo " +
		 * "   LEFT JOIN tblroom j ON i.strRoomNo = j.strRoomCode " +
		 * "   LEFT JOIN tblreservationdtl k ON k.strReservationNo=a.strReservationNo "
		 * + "   LEFT JOIN tblroom l ON l.strRoomCode = k.strRoomNo" +
		 * "  ,tblreceiptdtl b, tblguestmaster d, tblsettlementmaster e " +
		 * "  WHERE a.strReceiptNo=b.strReceiptNo AND b.strSettlementCode=e.strSettlementCode "
		 * + "  AND b.strCustomerCode = d.strGuestCode AND a.strClientCode='"+
		 * strClientCode+"' AND b.strClientCode='"+strClientCode+"' " +
		 * "  AND d.strClientCode='"
		 * +strClientCode+"' AND e.strClientCode='"+strClientCode
		 ...
		 * 
		 * 
		 * if(type.equals("Payment")) { sql +=
		 * " and a.strType in ('Payment','Deposit') "; }else
		 * if(type.equals("Refund")) { sql += " and a.strType='Refund Amt' "; }
		 * 
		 * sql += " Order by DATE(a.dteReceiptDate) asc ";
		 */
		String sql = "select a.strReceiptNo,IFNULL(DATE_FORMAT((b.dteCheckInDate),'%d-%m-%Y'),''),IFNULL(DATE_FORMAT((b.dteDepartureDate),'%d-%m-%Y'),''),IFNULL(d.strRoomDesc,' '),DATE(a.dteReceiptDate) as ReceiptDate, "
				+ " CONCAT(e.strFirstName,' ',e.strMiddleName,' ',e.strLastName), a.strAgainst, a.strFolioNo , g.strSettlementDesc,"
				+ " a.dblReceiptAmt, f.strRemarks"
				+ " FROM tblreceipthd a, tblcheckinhd b,tblcheckindtl c, tblroom d, tblguestmaster e, tblreceiptdtl f, tblsettlementmaster g "
				+ " where a.strCheckInNo = b.strCheckInNo and b.strCheckInNo=c.strCheckInNo and c.strRoomNo = d.strRoomCode "
				...
				+ "'"
				+ " UNION "
				+ "select a.strReceiptNo,IFNULL(DATE_FORMAT((m.dteArrivalDate),'%d-%m-%Y'),''),IFNULL(DATE_FORMAT((m.dteDepartureDate),'%d-%m-%Y'),''),IFNULL(d.strRoomDesc,' '),DATE(a.dteReceiptDate), "
				+ " CONCAT(e.strFirstName,' ',e.strMiddleName,' ',e.strLastName), a.strAgainst,a.strReservationNo, g.strSettlementDesc, "
				+ " a.dblReceiptAmt, f.strRemarks "
				+ " from tblreceipthd a, tblroom d, tblguestmaster e, tblreceiptdtl f, "
				+ " tblsettlementmaster g,tblreservationhd m, tblreservationdtl n "
				...
				+ "'"
				+ " UNION "
				+ "SELECT a.strReceiptNo,IFNULL(DATE_FORMAT((b.dteCheckInDate),'%d-%m-%Y'),''),IFNULL(DATE_FORMAT((h.dteBillDate),'%d-%m-%Y'),''),IFNULL(d.strRoomDesc,' '),DATE(a.dteReceiptDate),"
				+ " CONCAT(e.strFirstName,' ',e.strMiddleName,' ',e.strLastName), a.strAgainst,a.strBillNo ,g.strSettlementDesc,"
				+ " a.dblReceiptAmt, f.strRemarks from tblreceipthd a, tblcheckinhd b, tblroom d, tblguestmaster e, tblreceiptdtl f, "
				+ " tblsettlementmaster g, tblbillhd h, tblcheckindtl i where a.strCheckInNo = b.strCheckInNo and b.strCheckInNo = i.strCheckInNo "
				+ " and i.strRoomNo = d.strRoomCode and i.strGuestCode = e.strGuestCode and a.strReceiptNo = f.strReceiptNo "
				...
				+ "'"
				+ " UNION "
				+ "select a.strReceiptNo,'','','',DATE(a.dteReceiptDate), CONCAT(e.strFirstName,' ',e.strMiddleName,' ',e.strLastName), a.strAgainst, f.strCustomerCode, g.strSettlementDesc,"
				+ " a.dblReceiptAmt, f.strRemarks from tblreceipthd a, tblguestmaster e, tblreceiptdtl f,tblsettlementmaster g "
				+ " where a.strReceiptNo = f.strReceiptNo and f.strCustomerCode = e.strGuestCode and f.strSettlementCode = g.strSettlementCode	and a.strAgainst = 'Deposit'"
				+ " and DATE(a.dteReceiptDate) BETWEEN  '"
				+ fromDte
		...
		if (!listVoidBill.isEmpty()) {
			for (int i = 0; i < listVoidBill.size(); i++) {
				...
				objBean.setStrReceiptNo(arr2[0].toString());
				objBean.setDteCheckInDate(arr2[1].toString());
				objBean.setDteBillDate(arr2[2].toString());
				objBean.setStrRoomDesc(arr2[3].toString());
				// objBean.setStrFolioRoomDesc(arr2[4].toString());
				objBean.setDteReceiptDate(objGlobal.funGetDate("dd-MM-yyyy",
						arr2[4].toString()));
				objBean.setStrGuestName(arr2[5].toString());
				objBean.setStrType(arr2[6].toString());
				objBean.setStrDocCode(arr2[7].toString());
				objBean.setStrSettlement(arr2[8].toString());
				objBean.setDblAmount(Double.parseDouble(arr2[9].toString()));
				objBean.setStrRemark(arr2[10].toString());
	...
	public @ResponseBody List funBillPrintingDtl(HttpServletRequest request) {
		...
		String sql = "SELECT a.strBillNo, DATE(a.dteBillDate),c.strRoomDesc, CONCAT(e.strFirstName,' ',e.strMiddleName,' ',e.strLastName),"
				...
				+ " AND d.strCheckInNo=f.strCheckInNo AND DATE(a.dteBillDate) BETWEEN '"
				+ fromDte
				+ "' AND '"
				+ toDte
				+ "' "
				+ " AND a.strClientCode='"
				+ strClientCode + "'" + "  GROUP BY a.strBillNo ";
		...
		if (!listVoidBill.isEmpty()) {
			for (int i = 0; i < listVoidBill.size(); i++) {
				...
				objBean.setStrBillNo(arr2[0].toString());
				objBean.setDteBillDate(objGlobal.funGetDate("dd-MM-yyyy",
						arr2[1].toString()));
				...
				String sqlDisc = "select sum(a.dblDebitAmt) from tblbilldtl a where a.strBillNo='"
						+ arr2[0].toString()
						+ "' and a.strPerticulars='Room Tariff' and a.strClientCode='"
						+ strClientCode + "'";
				List listDisc = objGlobalService.funGetListModuleWise(sqlDisc,
						"sql");
				if (listDisc != null && listDisc.size() > 0) {
					if (listDisc.get(0) != null) {
						double dblDiscAmt = Double.parseDouble(listDisc.get(0)
								.toString());
						dblDiscAmt = (dblDiscAmt
								* Double.parseDouble(arr2[4].toString()) / 100);
						objBean.setDblDiscount(dblDiscAmt);
					}
				}

				String sqlTaxAmt = "select ifnull(sum(a.dblTaxAmt),0.0) from tblbilltaxdtl a where a.strBillNo='"
						+ arr2[0].toString()
						+ "' and a.strTaxCode like 'TC%' and a.strClientCode='"
						+ strClientCode + "'";
				List listTaxAmt = objGlobalService.funGetListModuleWise(
						sqlTaxAmt, "sql");
				if (listTaxAmt != null && listTaxAmt.size() > 0) {
					objBean.setDblTaxAmount(Double.parseDouble(listTaxAmt
							.get(0).toString()));
				}

				String sqlAdvanceAmt = "select a.dblReceiptAmt from tblreceipthd a where a.strCheckInNo='"
						+ arr2[5].toString()
						+ "' "
						+ "and a.strAgainst='Check-In' and a.strClientCode='"
						+ strClientCode + "';";
				List listAdvAmt = objGlobalService.funGetListModuleWise(
						sqlAdvanceAmt, "sql");
				if (listAdvAmt != null && listAdvAmt.size() > 0) {
					objBean.setDblAdvanceAmount(Double.parseDouble(listAdvAmt
							.get(0).toString()));
				} else {
					objBean.setDblAdvanceAmount(0);
				}

				String sqlPerticulars = "select a.strPerticulars from tblbilldtl a where a.strBillNo='"
						+ objBean.getStrBillNo()
						+ "' and a.strClientCode='"
						+ strClientCode + "'";
				List listPerticulars = objGlobalService.funGetListModuleWise(
						sqlPerticulars, "sql");
				if (listPerticulars != null && listPerticulars.size() > 0) {
					for (int p = 0; p < listPerticulars.size(); p++) {
						strPerticular = strPerticular
								+ listPerticulars.get(p).toString() + ",";
					}
	...
	public @ResponseBody List funloadHousekeepingSummary(
		...
		String sql = "UPDATE tblroom a, tblfoliohd b SET a.strRoomStatus = 'O'	WHERE a.strRoomCode = b.strRoomNo ";
		objWebPMSUtility.funExecuteUpdate(sql, "sql");

		String sql1 = "UPDATE tblroom a, tblfoliohd b SET a.strOccupiedStatus = 'OC' WHERE a.strRoomStatus='O' and  a.strRoomCode = b.strRoomNo and a.strStatus<>'Dirty' ";
		objWebPMSUtility.funExecuteUpdate(sql1, "sql");

		String sql2 = "UPDATE tblroom a, tblfoliohd b SET a.strOccupiedStatus = 'OD' WHERE a.strRoomStatus='O' and  a.strRoomCode = b.strRoomNo and a.strStatus='Dirty'";
		objWebPMSUtility.funExecuteUpdate(sql2, "sql");

		String sql3 = "UPDATE tblroom a, tblfoliohd b SET a.dteArrivalDate=b.dteArrivalDate WHERE a.strRoomCode = b.strRoomNo";
		objWebPMSUtility.funExecuteUpdate(sql3, "sql");

		String sql4 = "UPDATE tblroom a SET a.strRoomStatus = 'V' where a.strStatus='Free'";
		objWebPMSUtility.funExecuteUpdate(sql4, "sql");

		String sql5 = "UPDATE tblroom a SET a.strOccupiedStatus = 'VC' WHERE a.strRoomStatus='V'";
		objWebPMSUtility.funExecuteUpdate(sql5, "sql");

		List listRoomCode = objGlobalFunctionsService
				.funGetListModuleWise(
						"SELECT a.strRoomCode FROM tblblockroom a WHERE CURDATE() BETWEEN a.dteValidFrom AND a.dteValidTo ",
						"sql");
		if (listRoomCode != null && listRoomCode.size() > 0) {
			strRoomNo = listRoomCode.get(0).toString();

			String sql7 = "UPDATE tblroom a,tblblockroom b SET a.strRoomStatus = 'UR' WHERE a.strRoomCode='"
					+ strRoomNo + "' ";
			objWebPMSUtility.funExecuteUpdate(sql7, "sql");
		}

		String sql9 = "UPDATE tblroom a SET a.strOccupiedStatus='UR' WHERE a.strRoomStatus='UR' ";
		objWebPMSUtility.funExecuteUpdate(sql9, "sql");

		String sql6 = "UPDATE tblroom a,tblfoliohd b SET a.strOccupiedStatus = 'VD' where a.strStatus='Dirty' AND a.strOccupiedStatus<>'UR' AND  a.strRoomCode<>b.strRoomNo ";
		objWebPMSUtility.funExecuteUpdate(sql6, "sql");

		String sql10 = "UPDATE tblroom a SET a.strRoomStatus='V' WHERE a.strOccupiedStatus='VD'";
		objWebPMSUtility.funExecuteUpdate(sql10, "sql");

		List<clsPMSSalesFlashBean> listPayment = new ArrayList<clsPMSSalesFlashBean>();

		String sqlRoom = "SELECT a.strRoomDesc,IFNULL(a.strRoomStatus,' '),IFNULL(a.strOccupiedStatus,' '),"
				+ "ifnull(date_format(a.dteArrivalDate,'%d-%m-%y'),'NA'), date_format(b.dteBillDate,'%d-%m-%y')  FROM tblroom a , tblbillhd b "
				+ "WHERE a.strRoomCode=b.strRoomNo and a.strClientCode='"
				+ strClientCode + "'";

		List listHouseKeepingSummary = objGlobalService.funGetListModuleWise(
				sqlRoom, "sql");

		if (!listHouseKeepingSummary.isEmpty()) {
			for (int i = 0; i < listHouseKeepingSummary.size(); i++) {
				Object[] arr2 = (Object[]) listHouseKeepingSummary.get(i);
				clsPMSSalesFlashBean objBean = new clsPMSSalesFlashBean();

				objBean.setStrRoomDesc(arr2[0].toString());
				objBean.setStrRoomStatus(arr2[1].toString());
				objBean.setStrOccupiedStatus(arr2[2].toString());
				objBean.setDteArrivalDate(arr2[3].toString());
				objBean.setDteDepartureDate(arr2[4].toString());
	...
	public @ResponseBody List<clsPMSSalesFlashBean> funloadMonthwiseSale(
			HttpServletRequest request) throws ParseException {
		String strClientCode = request.getSession().getAttribute("clientCode")
				.toString();
		String fromDate = request.getParameter("frmDte").toString();
		...
		String[] arr1 = toDate.split("-");
		String toDte = arr1[2] + "-" + arr1[1] + "-" + arr1[0];
		String PMSDate = request.getSession().getAttribute("PMSDate")
				.toString();
	...
	public @ResponseBody List<clsPMSSalesFlashBean> funloadStaffWiseHousekeepingSummary(
			HttpServletRequest request) throws ParseException {
		String strClientCode = request.getSession().getAttribute("clientCode")
				.toString();
		String fromDate = request.getParameter("frmDte").toString();
		...
		listRoomCLeanCheck = objGlobalService.funGetListModuleWise(sqlData,
				"sql");
	...
	private ModelAndView funExportSettlementWisePMSSalesFlash(
			HttpServletRequest request) {
		String strClientCode = request.getSession().getAttribute("clientCode")
				.toString();
		...
		String sql = "select c.strSettlementDesc,sum(b.dblSettlementAmt) "
				...
				+ " and a.strClientCode=b.strClientCode and b.strClientCode=c.strClientCode AND a.strClientCode='"
				+ strClientCode + "' AND b.strClientCode='" + strClientCode
		...
		if (!listSettlementDtl.isEmpty()) {
			for (int i = 0; i < listSettlementDtl.size(); i++) {
				...
				dblTotalValue = new BigDecimal(df.format(Double
						.parseDouble(arr2[1].toString()))).add(dblTotalValue);
		...
		retList.add("SettlementWisePMSSalesFlashData_" + fromDte + "to" + toDte
				+ "_" + userCode);
		...
		return new ModelAndView("excelViewFromToDteReportName",
				"listFromToDateReportName", retList);
	}

	@SuppressWarnings({ "unchecked", "rawtypes" })
	@RequestMapping(value = "/exportRevenueHeadWisePMSSalesFlash", method = RequestMethod.GET)
	private ModelAndView funExportRevenueHeadWisePMSSalesFlash(
			HttpServletRequest request) {
		String strClientCode = request.getSession().getAttribute("clientCode")
				.toString();
		String userCode = request.getSession().getAttribute("usercode")
		...
		totalsList.add(dblTotalValue);
		retList.add("RevenueHeadWisePMSSalesFlashData_" + fromDte + "to"
				+ toDte + "_" + userCode);
		List titleData = new ArrayList<>();
	...
	private ModelAndView funExportTaxWisePMSSalesFlash(
			HttpServletRequest request) {
		String strClientCode = request.getSession().getAttribute("clientCode")
				.toString();
		String userCode = request.getSession().getAttribute("usercode")
		...
		if (!listTaxDtl.isEmpty()) {
			for (int i = 0; i < listTaxDtl.size(); i++) {
				...
				dblTotalValue = new BigDecimal(df.format(Double
						.parseDouble(arr2[1].toString()))).add(dblTotalValue);
				dblTaxTotalValue = new BigDecimal(df.format(Double
						.parseDouble(arr2[2].toString())))
						.add(dblTaxTotalValue);
			}
		}
		totalsList.add(dblTotalValue);
		...
		retList.add("TaxWisePMSSalesFlashData_" + fromDte + "to" + toDte + "_"
				+ userCode);
		List titleData = new ArrayList<>();
	...
	private ModelAndView funExportExpectedArrWisePMSSalesFlash(
			HttpServletRequest request) {
		String strClientCode = request.getSession().getAttribute("clientCode")
				.toString();
		String userCode = request.getSession().getAttribute("usercode")
				.toString();
		int serial__Number = 1;
		List retList = new ArrayList();
		List detailList = new ArrayList();
		List headerList = new ArrayList();
		List totalsList = new ArrayList();
		...
		BigDecimal dblTotalAmount = new BigDecimal(0);
		BigDecimal dblTotalValue = new BigDecimal(0);
		DecimalFormat df = new DecimalFormat("#.##");
		String sql = "SELECT a.strReservationNo, DATE_FORMAT(a.dteDateCreated,'%d-%m-%Y'), "
				+ " ifnull(CONCAT(e.strFirstName,' ',e.strMiddleName,' ',e.strLastName),''),g.strRoomTypeDesc,f.strRoomDesc,DATE_FORMAT(a.dteArrivalDate,'%d-%m-%Y'), DATE_FORMAT(a.dteDepartureDate,'%d-%m-%Y'),"
		...
		if (!listArrivalDtl.isEmpty()) {
			for (int i = 0; i < listArrivalDtl.size(); i++) {
				...
				dblTotalAmount = new BigDecimal(df.format(Double
						.parseDouble(arr2[7].toString()))).add(dblTotalAmount);
				dblTotalValue = new BigDecimal(df.format(Double
					.parseDouble(arr2[8].toString()))).add(dblTotalValue);
				detailList.add(DataList);
				serial__Number++;
			}
		}
		totalsList.add(dblTotalAmount);
		totalsList.add(dblTotalValue);
		retList.add("ExpectedArrWisePMSSalesFlashData_" + fromDte + "to"
				+ toDte + "_" + userCode);
		List titleData = new ArrayList<>();
	...
	private ModelAndView funExportExpectedDeptWisePMSSalesFlash(
			HttpServletRequest request) {
		String strClientCode = request.getSession().getAttribute("clientCode")
				.toString();
		String userCode = request.getSession().getAttribute("usercode")
		...
		retList.add("ExpectedDeptWisePMSSalesFlashData_" + fromDte + "to"
				+ toDte + "_" + userCode);
		List titleData = new ArrayList<>();
		...
		return new ModelAndView("excelViewFromDateTodateWithReportName",
				"listFromDateTodateWithReportName", retList);
	}

	@SuppressWarnings({ "unchecked", "rawtypes" })
	@RequestMapping(value = "/exportCheckInWisePMSSalesFlash", method = RequestMethod.GET)
	private ModelAndView funExportCheckInWisePMSSalesFlash(
			HttpServletRequest request) {
		String strClientCode = request.getSession().getAttribute("clientCode")
				.toString();
		String userCode = request.getSession().getAttribute("usercode")
		...
		/*
		 * String sql=
		 * "SELECT a.strCheckInNo,a.strType, DATE(a.dteArrivalDate),c.strRoomDesc,c.strRoomTypeDesc,CONCAT(d.strFirstName,' ',d.strMiddleName,' ',d.strLastName), "
		 * +" a.tmeArrivalTime" +
		 * " FROM tblcheckinhd a,tblcheckindtl b,tblroom c,tblguestmaster d,tblbillhd e "
		 * +" WHERE DATE(a.dteCheckInDate) BETWEEN '"+fromDte+"' AND '"+toDte+
		 * "' AND a.strCheckInNo=b.strCheckInNo AND b.strRoomNo=c.strRoomCode AND b.strGuestCode=d.strGuestCode "
		 * +
		...
		retList.add("CheckInWisePMSSalesFlashData_" + fromDte + "to" + toDte
				+ "_" + userCode);
		List titleData = new ArrayList<>();
	...
	private ModelAndView funExportCheckOutWisePMSSalesFlash(
			HttpServletRequest request) {
		String strClientCode = request.getSession().getAttribute("clientCode")
				.toString();
		String userCode = request.getSession().getAttribute("usercode")
		...
		if (!listCheckOutDtl.isEmpty()) {
			for (int i = 0; i < listCheckOutDtl.size(); i++) {
				...
				dblTotalValue = new BigDecimal(df.format(Double
						.parseDouble(arr2[6].toString()))).add(dblTotalValue);
				detailList.add(DataList);
				serial_number++;
		...
		retList.add("CheckOutWisePMSSalesFlashData_" + fromDte + "to" + toDte
				+ "_" + userCode);
		List titleData = new ArrayList<>();
	...
	private ModelAndView funExportCancelationWisePMSSalesFlash(
			HttpServletRequest request) {
		String strClientCode = request.getSession().getAttribute("clientCode")
				.toString();
		String userCode = request.getSession().getAttribute("usercode")
		...
		String sql = "SELECT a.strReservationNo, CONCAT(c.strFirstName,' ',c.strMiddleName,' ',c.strLastName) AS strGuestName, e.strBookingTypeDesc,h.strRoomTypeDesc,DATE_FORMAT(b.dteReservationDate,'%d-%m-%Y') AS dteReservationDate,DATE_FORMAT(a.dteCancelDate,'%d-%m-%Y') AS dteCancelDate,f.strRoomDesc, g.strReasonDesc, a.strRemarks "
				...
				+ "' AND a.strReservationNo=b.strReservationNo AND b.strCancelReservation='Y' AND b.strReservationNo=d.strReservationNo "
				+ " AND d.strGuestCode=c.strGuestCode AND b.strBookingTypeCode = e.strBookingTypeCode AND d.strRoomType=f.strRoomTypeCode "
				+ " AND a.strReasonCode=g.strReasonCode AND a.strClientCode=b.strClientCode AND h.strRoomTypeCode=d.strRoomType AND a.strClientCode='"
				+ strClientCode
				+ "' AND b.strClientCode='"
				+ strClientCode
				+ "' AND c.strClientCode='"
		...
		retList.add("CancelationWisePMSSalesFlashData_" + fromDte + "to"
				+ toDte + "_" + userCode);
		List titleData = new ArrayList<>();
	...
	private ModelAndView funNoShowWisePMSSalesFlash(HttpServletRequest request) {
		...
		String sql = "SELECT CONCAT(c.strFirstName,' ',c.strMiddleName,' ',c.strLastName),a.strReservationNo,a.strNoRoomsBooked, IFNULL(b.dblReceiptAmt,0) "
				...
				+ " on a.strReservationNo=b.strReservationNo AND a.strClientCode='"
				+ strClientCode
				+ "' AND b.strClientCode='"
				+ strClientCode
				...
				+ toDte
				+ "' "
				+ " and  a.strReservationNo Not IN(select strReservationNo from tblcheckinhd  where strClientCode='"
				+ strClientCode + "')";
		List listNoShowDtl = objGlobalService.funGetListModuleWise(sql, "sql");
		if (!listNoShowDtl.isEmpty()) {
			for (int i = 0; i < listNoShowDtl.size(); i++) {
		...
		totalsList.add(dblTotalValue);
		retList.add("NoShowWisePMSSalesFlashData_" + fromDte + "to" + toDte
				+ "_" + userCode);
		List titleData = new ArrayList<>();
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
	@RequestMapping(value = "/exportPMSRefund", method = RequestMethod.GET)
	private ModelAndView funPMSRefund(HttpServletRequest request) {
		String strClientCode = request.getSession().getAttribute("clientCode")
				.toString();
		String userCode = request.getSession().getAttribute("usercode")
				.toString();
		...
		String sql = "select a.strReceiptNo,IFNULL(DATE_FORMAT((a.dteReceiptDate),'%d-%m-%Y'),''), "
				+ " CONCAT(e.strFirstName,' ',e.strMiddleName,' ',e.strLastName), a.strAgainst,g.strSettlementDesc,"
				+ " a.dblReceiptAmt, f.strRemarks"
				+ " FROM tblreceipthd a, tblcheckinhd b,tblcheckindtl c, tblroom d, tblguestmaster e, tblreceiptdtl f, tblsettlementmaster g "
				+ " where a.strCheckInNo = b.strCheckInNo and b.strCheckInNo=c.strCheckInNo and c.strRoomNo = d.strRoomCode "
				+ " and c.strGuestCode = e.strGuestCode and a.strReceiptNo = f.strReceiptNo and f.strSettlementCode = g.strSettlementCode "
				...
				+ "' and a.strType LIKE '%Refund%' "
				+ " UNION "
				+ "select a.strReceiptNo,IFNULL(DATE_FORMAT((a.dteReceiptDate),'%d-%m-%Y'),''), CONCAT(e.strFirstName,' ',e.strMiddleName,' ',e.strLastName), a.strAgainst,g.strSettlementDesc,"
				+ " a.dblReceiptAmt, f.strRemarks from tblreceipthd a, tblguestmaster e, tblreceiptdtl f,tblsettlementmaster g "
				+ " where a.strReceiptNo = f.strReceiptNo and f.strCustomerCode = e.strGuestCode and f.strSettlementCode = g.strSettlementCode	and a.strAgainst = 'Deposit'"
				+ " and DATE(a.dteReceiptDate) BETWEEN  '"
				+ fromDte
		...
		if (!listVoidBill.isEmpty()) {
			for (int i = 0; i < listVoidBill.size(); i++) {
				...
				DataList.add(arr2[6].toString());
				dblTotalValue = new BigDecimal(df.format(Double
						.parseDouble(arr2[5].toString()))).add(dblTotalValue);
				detailList.add(DataList);
				serial_number++;
			}
		}
		...
		if (type.equals("Refund")) {
			retList.add("PMSRefundSlip_" + fromDte + "to" + toDte + "_"
					+ userCode);
	...
	private ModelAndView funPMSPaymentFlash(HttpServletRequest request) {
		...
		if (!listVoidBill.isEmpty()) {
			for (int i = 0; i < listVoidBill.size(); i++) {
				...
				DataList.add(arr2[3].toString());
				// DataList.add(arr2[4].toString());
				DataList.add(objGlobal.funGetDate("dd-MM-yyyy",
						arr2[4].toString()));
				DataList.add(arr2[5].toString());
				...
				DataList.add(arr2[10].toString());
				dblTotalValue = new BigDecimal(df.format(Double
						.parseDouble(arr2[9].toString()))).add(dblTotalValue);
				detailList.add(DataList);
				serial_number++;
			}
		}
		...
		List titleData = new ArrayList<>();
		if (type.equals("Payment")) {
			retList.add("PMSPaymentSlip_" + fromDte + "to" + toDte + "_"
					+ userCode);
		...
		headerList.add("RoomNO");
		headerList.add("Receipt Date");
		headerList.add("Guest Name");
		headerList.add("Type");
		headerList.add("Doc No");
		headerList.add("Settlement Type");
		headerList.add("Amount");
		headerList.add("Remark");
	...
	private ModelAndView funPMSBillPrinting(HttpServletRequest request) {
		 ...
		 * "and Date(a.dteBillDate) between '"
		 * +fromDte+"' and '"+toDte+"' and a.strClientCode='"
		 * +strClientCode+"' AND b.strClientCode='"
		 * +strClientCode+"' AND c.strClientCode='"
		 * +strClientCode+"' AND d.strClientCode='"
		 * +strClientCode+"' AND e.strClientCode='"
		 * +strClientCode+"' AND f.strClientCode='"
		 * +strClientCode+"' AND g.strClientCode='"
		 * +strClientCode+"' group by a.strBillNo";
		 */
		...
		if (!listVoidBill.isEmpty()) {
			for (int i = 0; i < listVoidBill.size(); i++) {
				...
				DataList.add(arr2[3].toString());
				// DataList.add(arr2[4].toString());
				dblTotalBillValue = new BigDecimal(df.format(Double
						.parseDouble(arr2[4].toString())))
						.add(dblTotalBillValue);
				String sqlDisc = "select sum(a.dblDebitAmt) from tblbilldtl a where a.strBillNo='"
						+ arr2[0].toString()
						+ "' and a.strPerticulars='Room Tariff' and a.strClientCode='"
						+ strClientCode + "'";
				...
				if (listDisc != null && listDisc.size() > 0) {
					if (listDisc.get(0) != null) {
						double dblDiscAmt = Double.parseDouble(listDisc.get(0)
								.toString());
						// dblDiscAmt = dblDiscAmt
						// -(dblDiscAmt*Double.parseDouble(arr2[5].toString())/100);
						dblDiscAmt = (dblDiscAmt
								* Double.parseDouble(arr2[4].toString()) / 100);
						DataList.add(dblDiscAmt);
						dblTotalDiscoun = new BigDecimal(df.format(Double
								.parseDouble(listDisc.get(0).toString())))
								.add(dblTotalDiscoun);
					}
				}
				...
				List listTaxAmt = objGlobalService.funGetListModuleWise(
						sqlTaxAmt, "sql");
				if (listTaxAmt != null && listTaxAmt.size() > 0) {
					DataList.add(Double.parseDouble(listTaxAmt.get(0)
							.toString()));
					dblTotalTaxAmt = new BigDecimal(df.format(Double
							.parseDouble(listTaxAmt.get(0).toString())))
							.add(dblTotalTaxAmt);
				}

				/*
				 * String sqlAdvanceAmt =
				 * "select a.dblReceiptAmt from tblreceipthd a where a.strCheckInNo='"
				 * +arr2[6].toString()+"' " +
				 * "and a.strAgainst='Check-In' and a.strClientCode='"
				 * +strClientCode+"';";
				 */
				...
				List listAdvAmt = objGlobalService.funGetListModuleWise(
						sqlAdvanceAmt, "sql");
				if (listAdvAmt != null && listAdvAmt.size() > 0) {
					dblTotalAdvanceAmt = new BigDecimal(df.format(Double
							.parseDouble(listAdvAmt.get(0).toString())))
							.add(dblTotalAdvanceAmt);
					DataList.add(Double.parseDouble(listAdvAmt.get(0)
							.toString()));
				} else {
					DataList.add(0);
				}
	...
	private ModelAndView funVoidBillWisePMSSalesFlash(HttpServletRequest request) {
		...
		retList.add("VoidBillWisePMSSalesFlashData_" + fromDte + "to" + toDte
				+ "_" + userCode);
		List titleData = new ArrayList<>();
	...
	public ModelAndView funGetBillPerticulars(Map<String, Object> model,
			@RequestParam("billNo") String strBillNo, HttpServletRequest request) {
		String strClientCode = request.getSession().getAttribute("clientCode")
				.toString();
		String urlHits = "1";
		String strBillPerticular = "";
		String sqlPerticular = "select a.strPerticulars from tblbilldtl a where a.strBillNo='"
				+ strBillNo + "' and a.strClientCode='" + strClientCode + "'";

		List listRecord = objGlobalService.funGetListModuleWise(sqlPerticular,
				"sql");
	...
	private ModelAndView funPMSHousekeepingSummary(HttpServletRequest request) {
		...
		if (!listRoomNo.isEmpty()) {
			for (int i = 0; i < listRoomNo.size(); i++) {
				Object[] arr2 = (Object[]) listRoomNo.get(i);
				List DataList = new ArrayList<>();
				DataList.add(serial_number);
				DataList.add(arr2[0].toString());
				DataList.add(arr2[1].toString());
		...
		retList.add("HouseKeeping Summary_" + fromDte + "to" + toDte + "_"
				+ userCode);
		List titleData = new ArrayList<>();
		titleData.add("HouseKeeping Report");
		retList.add(titleData);
		...
		headerList.add("Sr.No");
		headerList.add("Room No");
		headerList.add("Room Status");
		headerList.add("Occupied Status");
		headerList.add("Arrival Date");
		headerList.add("CheckOut Date");
	...
	private ModelAndView funexportPMSStaffWiseHousekeepingSummary(
			HttpServletRequest request) throws ParseException {
		...
		List listRoomCLeanCheck = objGlobalService.funGetListModuleWise(
				sqlData, "sql");
		...
		retList.add("Staffwise Housekeeping_" + fromDte + "to" + toDte + "_"
				+ userCode);
		List titleData = new ArrayList<>();
	...
	public @ResponseBody List<clsPMSSalesFlashBean> funloadGuestData(
			HttpServletRequest request) throws ParseException {
		String strClientCode = request.getSession().getAttribute("clientCode")
				.toString();
		...
		List<clsPMSSalesFlashBean> listMain = new ArrayList<clsPMSSalesFlashBean>();
		List listGuestData = objGlobalService.funGetListModuleWise(sqlData,
				"sql");
	...
	private ModelAndView funexportGuestLedgerData(HttpServletRequest request)
		...
		filterData.add(" ");
		/*
		 * filterData.add(fromDate); filterData.add("To Date");
		 * filterData.add(toDate);
		 */
		retList.add(filterData);
	...
	public @ResponseBody List<clsPMSSalesFlashBean> funloadAvailableRooms(
			HttpServletRequest request) throws ParseException {
		String strClientCode = request.getSession().getAttribute("clientCode")
				.toString();
		String fromDate = request.getParameter("frmDte").toString();
		...
		String sqlData = " SELECT a.strRoomDesc,b.strRoomTypeDesc FROM tblroom a,tblroomtypemaster b "
				...
				+ " AND a.strStatus = 'Free'  AND a.strClientCode='"
				+ strClientCode + "' ORDER BY a.strRoomDesc ASC ";
		List<clsPMSSalesFlashBean> listMain = new ArrayList<clsPMSSalesFlashBean>();
		List listAvailableRooms = objGlobalService.funGetListModuleWise(
				sqlData, "sql");
	...
	private ModelAndView funExportAvailableRoom(HttpServletRequest request) {
		...
		retList.add("AvailableRoomWiseFlashData_" + fromDte + "to" + toDte
				+ "_" + userCode);
		List titleData = new ArrayList<>();
	...
	public void funGenerateGuestLedgerPrintingReport(
			@RequestParam("GuestCode") String strGuestCode,
		...
		String reportName = servletContext
				.getRealPath("/WEB-INF/reports/webpms/rptGuestLedgerPrinting.jrxml");
		...
		/*
		 * folioBean.setDteDocDate(" "); folioBean.setStrDocNo(" ");
		 * folioBean.setStrPerticulars("Opening Balance"); double
		 * openingBalance=
		 * objPMSUtilityFunctions.funGetOpeningBalanceForOneGuest(
		 * strGuestCode);; if(openingBalance>0) {
		 * folioBean.setDblDebitAmt(openingBalance);
		 * folioBean.setDblCreditAmt(0.00); } else {
		 * folioBean.setDblCreditAmt(openingBalance);
		 * folioBean.setDblDebitAmt(0.00); }
		 * folioBean.setDblBalanceAmt(openingBalance);
		 * folioBean.setDblQuantity(0.00); dataList.add(folioBean);
		 */
		...
		String sql = "	select  a.docNo,a.docDate,a.settlementName, a.modeOfOperation  ,a.totalAmt,a.GuestCode"
				...
				+ " and a.strCustomerCode='"
				+ strGuestCode
				+ "' and a.dblSettlementAmt <>0 and strType in ('Payment','Deposit') "
				+ " union All "
				...
				+ " and a.strCustomerCode='"
				+ strGuestCode
				+ "' and a.dblSettlementAmt <> 0 and strType='Refund Amt'"
				+ " union All "
				...
				+ " AS GuestCode"
				+ " FROM tblbillhd a"
				+ " where a.strGuestCode='"
				+ strGuestCode
				+ "'  and a.dblGrandTotal <> 0"
				+ " union All"
				...
				+ " and b.strGuestCode='"
				+ strGuestCode + "' and a.dblDebitAmt <>0" + "  union All ";

		sql += "SELECT a.strFolioNo AS docNo,DATE_FORMAT(DATE(a.dteArrivalDate),'%d-%m-%Y') AS docDate, b.strTaxDesc AS settlementName ,"
				+ "	'Folio' AS modeOfOperation,b.dblTaxAmt AS totalAmt,a.strGuestCode AS GuestCode "
				+ "	FROM tblfoliohd  a, tblfoliotaxdtl b"
				+ "	WHERE a.strFolioNo = b.strFolioNo AND a.strGuestCode='"
				+ strGuestCode + "'";

		sql += ")  a";

		sql += " order by a.docDate ASC ";
		...
		folioDtlList
				...
				.forEach(
						folioArr -> {
							...
							folioPrintingBean
									.setStrDocNo(((Object[]) folioArr)[0]
											.toString());
							folioPrintingBean
									.setDteDocDate(((Object[]) folioArr)[1]
											.toString());
							String particulars = ((Object[]) folioArr)[2]
									.toString();
							folioPrintingBean.setStrPerticulars(particulars);
							double debitAmount = 0;
							double creditAmount = 0;
							if (((Object[]) folioArr)[3].toString().equals(
									"Refund Amt")
									|| ((Object[]) folioArr)[3].toString()
											.equals("Bill")
									|| ((Object[]) folioArr)[3].toString()
											.equals("Folio")) {
		...
		clsPropertySetupHdModel objPropertySetupModel = objPropertySetupService
				.funGetPropertySetup(propertyCode, clientCode);

		reportParams.put("pCompanyName", companyName);
		reportParams.put("pAddress1",
				objSetup.getStrAdd1() + "," + objSetup.getStrAdd2() + ","
						+ objSetup.getStrCity());
		reportParams.put("pAddress2",
				objSetup.getStrState() + "," + objSetup.getStrCountry() + ","
						+ objSetup.getStrPin());
		reportParams.put("pContactDetails", "");
		reportParams.put("strImagePath", imagePath);

		List listGuestData = objGuestMasterDao.funGetGuestMaster(strGuestCode,
				clientCode);
		clsGuestMasterHdModel objGuestMasterModel = (clsGuestMasterHdModel) listGuestData
				.get(0);
		reportParams.put("pGuestName", objGuestMasterModel.getStrGuestPrefix()
				+ " " + objGuestMasterModel.getStrFirstName() + " "
				+ objGuestMasterModel.getStrMiddleName() + " "
				+ objGuestMasterModel.getStrLastName());
		...
		if (listguest.size() > 0) {
			...
			if (arrGuest[0].toString().equalsIgnoreCase("Permanent")) { // check
				 ...
				 * arrGuest[7].toString() + "," + arrGuest[8].toString() + "," +
				 * arrGuest[9].toString() + "," + arrGuest[10].toString();
				 */
				guestAddr = arrGuest[6].toString();
				 ...
				 * arrGuest[12].toString() + "," + arrGuest[13].toString() + ","
				 * + arrGuest[14].toString() + "," + arrGuest[15].toString();
				 */
				guestAddr = arrGuest[11].toString();
				 ...
				 * arrGuest[2].toString() + "," + arrGuest[3].toString() + "," +
				 * arrGuest[4].toString() + "," + arrGuest[5].toString();
				 */
				guestAddr = arrGuest[1].toString();
		...
		try {
			...
			if (jp != null) {
				...
				resp.setHeader("Content-Disposition",
						"inline;filename=GuestLedger.pdf");
	...
	@RequestMapping(value = "/rptExpectedArrWiseDtlInPDF", method = RequestMethod.GET)
	public void funExpectedArrWiseDtlInPDF(HttpServletRequest req,
			HttpServletResponse resp) {
		String fromDate = req.getParameter("frmDte").toString();
		String[] arr = fromDate.split("-");
		String strClientCode = req.getSession().getAttribute("clientCode")
				.toString();
		String fromDte = arr[2] + "-" + arr[1] + "-" + arr[0];
		...
		String reportName = servletContext
				.getRealPath("/WEB-INF/reports/webpms/rptExpectedArrivalListFromSalesFlash.jrxml");
		String imagePath = servletContext
				.getRealPath("/resources/images/company_Logo.png");
		...
		try {
			...
			String sql = "SELECT a.strReservationNo, DATE_FORMAT(a.dteDateCreated,'%d-%m-%Y') as dteResDate, "
					+ " ifnull(CONCAT(e.strFirstName,' ',e.strMiddleName,' ',e.strLastName),'') as strGuestName,"
					+ "  g.strRoomTypeDesc as strRoomType,f.strRoomDesc as strRoomNo,DATE_FORMAT(a.dteArrivalDate,'%d-%m-%Y') as arrivalDate, DATE_FORMAT(a.dteDepartureDate,'%d-%m-%Y') as departureDate,IFNULL(j.DepositAmt,0) as DepositAmt,IFNULL(i.dblReceiptAmt,0) as recieptAmt "
					+ " FROM tblreservationhd a "
					+ " LEFT OUTER JOIN tblreservationdtl h ON a.strReservationNo=h.strReservationNo "
					+ " LEFT OUTER JOIN tblreceipthd d ON a.strReservationNo=d.strReservationNo and d.strFlagOfAdvAmt='Y'"
					+ " LEFT OUTER JOIN tblguestmaster e ON e.strGuestCode=a.strGuestCode "
			...
			JasperDesign jd = JRXmlLoader.load(reportName);
			JRDesignQuery occupancy = new JRDesignQuery();
			occupancy.setText(sql);
			Map<String, JRDataset> datasetMap = jd.getDatasetMap();
			JRDesignDataset taxSumDataset = (JRDesignDataset) datasetMap
					.get("dsBillWiseInvoice");
			taxSumDataset.setQuery(occupancy);
			JasperReport jr = JasperCompileManager.compileReport(jd);
			JasperPrint jp = JasperFillManager
					.fillReport(jr, reportParams, con);
			List<JasperPrint> jprintlist = new ArrayList<JasperPrint>();
	...
	@RequestMapping(value = "/rptExpectedDeptWiseDtlInPDF", method = RequestMethod.GET)
	public void funExpectedDeptWiseDtlInPDF(HttpServletRequest req,
			HttpServletResponse resp) {
		String fromDate = req.getParameter("frmDte").toString();
		String strClientCode = req.getSession().getAttribute("clientCode")
				.toString();
		...
		String reportName = servletContext
				.getRealPath("/WEB-INF/reports/webpms/rptExpectedDepaturelListFromSalesFlash.jrxml");
		String imagePath = servletContext
				.getRealPath("/resources/images/company_Logo.png");
		...
		try {
			...
			String sql = "SELECT a.strCheckInNo as strCheckInNo ,a.strType as strBookingType , DATE_FORMAT(a.dteDepartureDate,'%d-%m-%Y') as depatureDate "
					+ ",c.strRoomDesc as strRoomNo ,c.strRoomTypeDesc as strRoomType ,"
					+ " CONCAT(d.strFirstName,' ',d.strMiddleName,'',d.strLastName) as strGuestName "
					+ " FROM tblcheckinhd a,tblcheckindtl b,tblroom c,tblguestmaster d "
					+ " WHERE a.strCheckInNo=b.strCheckInNo AND b.strRoomNo=c.strRoomCode AND b.strGuestCode=d.strGuestCode "
					+ " AND a.strClientCode='"
					+ strClientCode
		...
		/*
		 * List
		 * listExpectedDeptDtl=objGlobalService.funGetListModuleWise(sql,"sql");
		 * if(!listExpectedDeptDtl.isEmpty()) { for(int
		 * i=0;i<listExpectedDeptDtl.size();i++) { Object[] arr2=(Object[])
		 * listExpectedDeptDtl.get(i); clsPMSSalesFlashBean objBean=new
		 * clsPMSSalesFlashBean(); objBean.setStrCheckInNo(arr2[0].toString());
		 * objBean.setStrBookingType(arr2[1].toString());
		 * objBean.setDteDepartureDate(arr2[2].toString());
		 * objBean.setStrRoomDesc(arr2[3].toString());
		 * objBean.setStrRoomType(arr2[4].toString());
	...
	@RequestMapping(value = "/loadIncomeHeadDtl", method = RequestMethod.GET)
	public @ResponseBody List funIncomeHeaddtlDtl(HttpServletRequest request) {
		String strClientCode = request.getSession().getAttribute("clientCode")
				.toString();
		String fromDate = request.getParameter("frmDte").toString();
		String[] arr = fromDate.split("-");
		...
		List<clsPMSSalesFlashBean> listofIncomeHdInDtl = new ArrayList<clsPMSSalesFlashBean>();
		String sql = "SELECT a.strFolioNo,DATE_FORMAT(a.dteDocDate,'%d-%m-%Y'),a.strDocNo,a.strPerticulars,a.dblQuantity,a.dblDebitAmt,a.strUserEdited,DATE_FORMAT(a.dteDateEdited,'%d-%m-%Y') "
				+ " FROM tblfoliodtl a WHERE a.strRevenueType='Income Head'  AND a.dteDocDate BETWEEN '"
				+ fromDte + "' and '" + toDte + "' ";
		List listIncomeHdDtl = objGlobalService
				.funGetListModuleWise(sql, "sql");
		if (!listIncomeHdDtl.isEmpty()) {
			for (int i = 0; i < listIncomeHdDtl.size(); i++) {
				Object[] arr2 = (Object[]) listIncomeHdDtl.get(i);
				clsPMSSalesFlashBean objBean = new clsPMSSalesFlashBean();
				objBean.setStrFolioNo(arr2[0].toString());
				objBean.setDteDocDate(arr2[1].toString());
				objBean.setStrDocCode(arr2[2].toString());
				objBean.setStrPerticular(arr2[3].toString());
				objBean.setDblQuantity(Double.parseDouble(arr2[4].toString()));
				objBean.setDblAmount(Double.parseDouble(arr2[5].toString()));
				objBean.setStrUserEdited(arr2[6].toString());
				objBean.setDteDateEdited(arr2[7].toString());
				listofIncomeHdInDtl.add(objBean);

			}
		}
		return listofIncomeHdInDtl;
	}

	@RequestMapping(value = "/rptCheckInWisePMSSalesFlashInPDF", method = RequestMethod.GET)
	public void funCheckInWisePMSSaleshInPDF(HttpServletRequest req,
			HttpServletResponse resp) {
		String fromDate = req.getParameter("frmDte").toString();
		String strClientCode = req.getSession().getAttribute("clientCode")
				.toString();
		...
		String reportName = servletContext
				.getRealPath("/WEB-INF/reports/webpms/rptCheckInWisePMSSalesFlashInPDF.jrxml");
		String imagePath = servletContext
				.getRealPath("/resources/images/company_Logo.png");
