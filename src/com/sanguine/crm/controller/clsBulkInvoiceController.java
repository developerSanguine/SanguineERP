	...
	public @ResponseBody List funAssignFields(@RequestParam("CustCode") String strCustCode,@RequestParam("frmDate") String dteFrmDate,@RequestParam("toDate") String dteToDate,HttpServletRequest req)
	{
		...
		/*	String sqlSO=" SELECT a.strSOCode,a.dteSODate,a.strCustCode,a.dblSubTotal,c.strPName,a.strStatus "
			...
			String sqlSO="SELECT a.strSOCode,a.dteSODate,a.strCustCode,a.dblTotal,c.strPName,a.strStatus"
					...
					+ " AND a.strLocCode='"+strLocCode+"' "
					+ " AND DATE(a.dteSODate) BETWEEN '"+dteFrmDateSql+"' AND '"+dteToDateSql+"'  "
					+ " AND a.dblSubTotal>0 AND a.strSOCode NOT IN(SELECT a.strSOCode FROM tblinvsalesorderdtl a, tblsalesorderhd b  "
					+ " where a.strSOCode = b.strSOCode and DATE(b.dteSODate) BETWEEN '"+dteFrmDateSql+"' AND '"+dteToDateSql+"' )"
					+ " AND a.strClientCode='"+clientCode+"' AND a.strCustCode IN  ("+strSupplierCode+ ") " ;
...
*/
	...
	*/
		/*String sqlProdDtl = " select  c.strProdName,c.strProdNameMarathi,b.dblQty,"
				...
				+ " IFNULL(d.strBAdd2,''), " + " IFNULL(d.strBState,''),IFNULL(d.strBPin,'') ,IFNULL(d.strSAdd1,''),IFNULL(d.strSAdd2,''), " + " IFNULL(d.strSState,''),IFNULL(d.strSPin,'') "
				+ ",IFNULL(d.strGSTNo,''),b.dblProdDiscAmount,b.dblWeight,f.strSGName,IFNULL(d.strEmail,''), IFNULL(d.strMobile,''),"
				+ " f.intSortingNo  "//29
				+ " from tblinvoicehd a left outer join tblinvoicedtl b on a.strInvCode=b.strInvCode   " + ""
						+ " left outer join tblproductmaster c  "
				+ " on b.strProdCode=c.strProdCode left outer join tblpartymaster d on a.strCustCode=d.strPCode " + ""
		...
		Map<Double, Double> hmSGSTCalculateTax = new HashMap<Double, Double>();
		  ...
		  //Changes for speed 04-09-2020
		   clsTaxBeanForInvoice objTaxBean=new clsTaxBeanForInvoice();
			...
			String sqlQuery = " select a.strProdCode,b.strTaxCode,b.dblPercent,a.dblValue,b.strShortName,a.dblTaxableAmt ,b.dblAbatement " + " "
			...
			if (listProdTax.size() > 0)
			{
				for (int j = 0; j < listProdTax.size(); j++)
				{
					...
					if(mapTaxForProduct.containsKey(objGST[0].toString()))
					{
						objTaxBean=new clsTaxBeanForInvoice();
						taxList=mapTaxForProduct.get(objGST[0].toString());
						...
						mapTaxForProduct.put(objGST[0].toString(), taxList);
					}
					else
					{
		...
		if (listProdDtl.size() > 0)
		{
			for (int i = 0; i < listProdDtl.size(); i++)
			{
				...
				if(mapTaxForProduct.containsKey(obj[11].toString()))   
			         {
			        	    double cGStAmt = 0.0;
							double sGStAmt = 0.0;
							List<clsTaxBeanForInvoice> listTaxForProduct =mapTaxForProduct.get(obj[11].toString());
