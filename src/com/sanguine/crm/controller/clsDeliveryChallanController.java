...
public class clsDeliveryChallanController {
	 ...
	 * @return
	 */
	@RequestMapping(value = "/frmSalesOrderForDC", method = RequestMethod.GET)
	public ModelAndView funOpenSalesOrderForDC(Map<String, Object> model, HttpServletRequest req)
	{
		
		String custCode = req.getParameter("strCustCode");
		model.put("strCustCode", custCode);
		return new ModelAndView("frmSalesOrderForDC", "command", new clsReportBean());

	}
	
	@SuppressWarnings({ "rawtypes", "unused" })
	@RequestMapping(value = "/loadSOforDeliveryChallan", method = RequestMethod.GET)
	public @ResponseBody List funLoadSOForDeliveryChallan(HttpServletRequest request)
	{
				...
				//Query for partial Sales Order
				/*String sqlData="select  a.strSOCode,a.dteSODate,a.strCustPONo,b.strPName,c.strLocName,a.strStatus   "
							+ " from tblsalesorderhd a,tblpartymaster b,tbllocationmaster c     "
							...
							+ " and strSOCode IN "
							+ " (select distinct a.strSOCode from tblsalesorderdtl a left outer join "
							+ " (SELECT a.strSOCode AS SOCode, b.strProdCode, SUM(b.dblQty) AS SOQty "
							+ " FROM tbldeliverychallanhd a INNER JOIN tbldeliverychallandtl b ON a.strDCCode = b.strDCCode "
							+ " and b.strClientCode='"+clientCode+"' WHERE (a.strAgainst = 'Sales Order') and a.strClientCode='"+clientCode+"' "
							+ " GROUP BY a.strSOCode, b.strProdCode) b on a.strSOCode = b.SOCode and a.strProdCode = b.strProdCode "
							+ " and a.strClientCode='"+clientCode+"'where  a.dblQty > ifnull(b.SOQty,0)) "
							+ " and a.strAuthorise='Yes' and a.strCustCode='"+custCode+"' and a.strClientCode='"+clientCode+"'";*/
				String sqlData="select  a.strSOCode,DATE_FORMAT(a.dteSODate, '%d-%m-%Y'),a.strCustPONo,c.strLocName,a.strStatus  " 
							   + " from tblsalesorderhd a,tblpartymaster b,tbllocationmaster c  "     
							   + " where a.strCustCode=b.strPCode and a.strLocCode=c.strLocCode and a.strClientCode='"+clientCode+"'  " 
							   + " and  a.strClientCode=b.strClientCode and a.strClientCode=c.strClientCode and c.strPropertyCode='"+propCode+"'    " 
							   + " and a.strCustCode='"+custCode+"' and a.strClientCode='"+clientCode+"'  " 
							   + " and strSOCode NOT IN (select a.strSOCode from tbldeliverychallanhd a  " 
