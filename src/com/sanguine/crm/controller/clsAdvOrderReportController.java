...
public class clsAdvOrderReportController {
	...
	public ModelAndView funOpenAdvanceOrderReport40Col(Map<String, Object> model, HttpServletRequest request) {
	...
	@RequestMapping(value = "/rptOrderDispatchReport", method = RequestMethod.POST)
	public void funCallAdvOrderReport40Col(clsReportBean objBean, String type,HttpServletResponse resp, HttpServletRequest req) {
		Connection con = null;
			...
			{
				reportName = servletContext.getRealPath("/WEB-INF/reports/webcrm/rptOrderDispatchDateReport.jrxml");
			}
			else
			{
				reportName = servletContext.getRealPath("/WEB-INF/reports/webcrm/rptOrderDispatchDateReport.jrxml");
...
sqlAdvOrd = " select i.strSGName,   g.strProdCode,g.strProdName,b.dblQty AS dblQty, b.dblWeight AS dblWeight,b.strSOCode,ifnull(c.strCharCode,''),ifnull(c.strCharValue,''),e.strPName,  DATE_FORMAT(a.dteSODate,'%d-%m-%Y'),b.strRemarks,ifnull(d.strCharName,''),ifnull(b.strMessage,''),ifnull(b.strShape,''),IfNull(g.strProdCode,''), DATE_FORMAT(a.dteFulmtDate,'%d-%m-%Y') " + " from tblsalesorderhd a "
				...
				sqlAdvOrd = "  select i.strSGName,   g.strProdCode,g.strProdName,b.dblQty AS dblQty, b.dblWeight AS dblWeight,b.strSOCode,ifnull(c.strCharCode,''),  ifnull(c.strCharValue,''),e.strPName,  DATE_FORMAT(a.dteSODate,'%d-%m-%Y'),b.strRemarks,ifnull(d.strCharName,''),ifnull(b.strMessage,''),ifnull(b.strShape,''),DATE_FORMAT(a.dteFulmtDate,'%d-%m-%Y')  " + " from tblsalesorderhd a left outer join tblsalesorderdtl b on a.strSOCode=b.strSOCode   "
			...
			for (int j = 0; j < listProdDtl.size(); j++) {
				...
				objProdCompilationBean.setDteFulmtDate(prodArr[14].toString());
			 ...
			 * if (objBean.getStrExportType().equalsIgnoreCase("Yes")) {
			 * 
			 * clsProductionComPilationBean objProdBean = null; for (int a = 0; a <
			 * fieldList.size(); a++) { String prodImage = ""; objProdBean =
			 * (clsProductionComPilationBean) fieldList.get(a);
			 * 
			 * List<byte[]> listBean =
			 * objAdvOrderService.funGetImageAdvOrder(objProdBean.getStrSOCode(),
			 * objProdBean.getStrItemCode(), clientCode); //Blob image = null; byte[]
			 * imgData = null; String encodedImage =""; try {
			 * 
			 * //Object prodimage = listBean.get(0); //image = (Blob) prodimage; if (null !=
			 * listBean.get(0) && listBean.get(0).length > 0) { imgData = listBean.get(0);
			 * try { //imgData = Base64.getDecoder().decode(imgData);// decoding // of //
			 * byte //encodedImage = Base64.getEncoder().encodeToString(imgData); } catch
			 * (Exception ex) { ex.printStackTrace(); } File imgFolder = new
			 * File(System.getProperty("user.dir") + "\\ProductImageIcon"); if
			 * (!imgFolder.exists()) { if (imgFolder.mkdir()) {
			 * System.out.println("Directory is created! " + imgFolder.getAbsolutePath()); }
			 * else { System.out.println("Failed to create directory!"); } } File
			 * fileUserImage = new File(System.getProperty("user.dir") +
			 * "\\ProductImageIcon\\" + objProdBean.getStrPName() + "
			 * _" + objProdBean.getStrRemark() + "_" + objProdBean.getStrProdCode() + "
			 * _" + objProdBean.getDblWeight() + ".jpg"); prodImage =
			 * System.getProperty("user.dir") +
			 * "\\ProductImageIcon\\"+ objProdBean.getStrPName() + "
			 * _" + objProdBean.getStrRemark() + "_" + objProdBean.getStrProdCode() + "
			 * _" + objProdBean.getDblWeight() + "
			 * .jpg";   //AAYUSH FOODS_AB00071_P0000215_0.0.png"; if
			 * (fileUserImage.exists()) { fileUserImage.delete(); }
			 * fileUserImage.createNewFile(); FileOutputStream fos = new
			 * FileOutputStream(fileUserImage); fos.write(imgData); fos.close();
			 * 
			 * objProdBean.setStrImage(prodImage);
			 * 
			 * }
			 * 
			 * } catch (Exception ex) { objProdBean.setStrImage(prodImage);
			 * ex.printStackTrace();
			 * 
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	@RequestMapping(value = "/rptAdvOrderReport40Col", method = RequestMethod.POST)
	public void funCallAdvOrderReport40Col(clsReportBean objBean, HttpServletResponse resp, HttpServletRequest req) {
		Connection con = null;
		...
		try {
			...
			if (objBean.getStrExportType().equalsIgnoreCase("Yes")) {
				reportName = servletContext.getRealPath("/WEB-INF/reports/webcrm/rptAdvanceOrderReport40ColSlip.jrxml");
			}
