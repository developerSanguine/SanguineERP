...
import com.sanguine.model.clsPropertyMaster;
import com.sanguine.crm.bean.clsTaxBeanForInvoice;
@Controller
...
//					}
			...
			objGlobalFunctions.funUpdateBatchQty(objHDModel.getStrInvCode(), clientCode, "INV", req);
			...
			if (objCompModel.getStrWebBookModule().equals("Yes"))
			{
				...
				objGlobalFunctions.funSendWhatsAppMessage(objHDModel.getStrInvCode(), "Invoice");
...
*/
	...
	String sqlProdDtl = "select c.strProdName,c.strProdNameMarathi,b.dblQty, IFNULL(b.dblPrice,0.00),c.dblMRP, IFNULL(b.dblBillRate,0.00) AS dblPrice,  "
						+ " a.dteInvDate, IFNULL(d.strPName,''), 'DC Code', 'DC Date', 'PO No', b.strProdCode,c.strHSNCode,g.dblValue AS discAmt,  "
						+ " IFNULL(d.strBAdd1,''), IFNULL(d.strBAdd2,''), IFNULL(d.strBState,''), IFNULL(d.strBPin,''), IFNULL(d.strSAdd1,''),  "
						+ " IFNULL(d.strSAdd2,''), IFNULL(d.strSState,''), IFNULL(d.strSPin,''), IFNULL(d.strGSTNo,''),b.dblProdDiscAmount,  "
						+ " b.dblWeight,f.strSGName, IFNULL(d.strEmail,''), IFNULL(d.strMobile,''),f.intSortingNo,a.dblExtraCharges,  "
						+ " IFNULL(b.strRemarks,'') ,d.strTaxNo1 "
						+ " from tblinvoicehd a, tblinvoicedtl b, tblproductmaster c, tblpartymaster d, tblsubgroupmaster f, tblinvprodtaxdtl g   "
						+ " where a.strInvCode = b.strInvCode  "
						+ " and b.strProdCode = c.strProdCode  "
						+ " and a.strCustCode = d.strPCode  "
						+ " and c.strSGCode = f.strSGCode  "
						+ " and a.strInvCode = g.strInvCode  "
						+ " AND a.strCustCode=g.strCustCode  "
						+ " and a.strInvCode='"+InvCode+"' AND a.strClientCode='"+clientCode+"'  "
						+ " AND a.strCustCode=g.strCustCode AND b.dblWeight=g.dblWeight  "
						+ " AND b.strProdCode=g.strProdCode AND g.strDocNo='Disc'  "
						+ " ORDER BY f.intSortingNo,f.strSGName,c.strProdName; "	;			
	String bAddress = "",bAddress1="";
	String bState = "";
	String bPin = "";
	String sAddress = "",sAddress1="";
	String sState = "";
	String sPin = "";
	String custGSTNo = "";
	String custEmailID="",custMobileNo="";
	String taxCode1="",taxCode2="";
	double totalInvoiceValue = 0.0;
	...
	clsTaxBeanForInvoice objTaxBean=new clsTaxBeanForInvoice();
	Map<String,List> mapTaxForProduct=new HashMap<String,List>();
	double totalTaxAmt=0;
	
	
	
	List taxList=new ArrayList<>();
	String sqlQuery = " select a.strProdCode,b.strTaxCode,b.dblPercent,a.dblValue,b.strShortName,a.dblTaxableAmt ,b.dblAbatement,a.dblWeight " + " "
			+ " from tblinvprodtaxdtl a,tbltaxhd b " + ""
			+ " where a.strDocNo=b.strTaxCode and a.strInvCode='" + InvCode + "' " + "  and a.strClientCode='" + clientCode + "' "
			+ " group by a.strProdCode,b.strTaxCode,b.dblPercent,a.dblValue,b.strShortName,a.dblTaxableAmt,b.dblAbatement ";

	List listProdTax = objGlobalFunctionsService.funGetDataList(sqlQuery, "sql");
	
	if (listProdTax.size() > 0)
	{
		for (int j = 0; j < listProdTax.size(); j++)
		{
			Object[] objGST = (Object[]) listProdTax.get(j);
			if(mapTaxForProduct.containsKey(objGST[0].toString()+" "+df.format(Double.parseDouble(objGST[7].toString()))))
			{
				objTaxBean=new clsTaxBeanForInvoice();
				taxList=mapTaxForProduct.get(objGST[0].toString()+" "+df.format(Double.parseDouble(objGST[7].toString())));
				objTaxBean.setStrTaxCode(objGST[1].toString());
				objTaxBean.setDblPercent(Double.parseDouble(objGST[2].toString()));
				objTaxBean.setDblValue(Double.parseDouble(objGST[3].toString()));
				objTaxBean.setStrShortName(objGST[4].toString());
				objTaxBean.setDblTaxableAmt(Double.parseDouble(objGST[5].toString()));
				objTaxBean.setDblAbatement(Double.parseDouble(objGST[6].toString()));
				taxList.add(objTaxBean);
				mapTaxForProduct.put(objGST[0].toString()+" "+df.format(Double.parseDouble(objGST[7].toString())), taxList);
			}
			else
			{
	...
	if (listProdDtl.size() > 0)
	{
		for (int i = 0; i < listProdDtl.size(); i++)
		{
			...
			// objDtlBean.setDblDisAmt(Double.parseDouble(obj[13].toString())*Double.parseDouble(obj[2].toString()));
			//bAddress = obj[14].toString() + " " + obj[15].toString();
			bAddress = obj[14].toString() ;
			bAddress1=obj[15].toString();
			bState = obj[16].toString();
			bPin = obj[17].toString();
			objDtlBean.setDblDisAmt(Double.parseDouble(obj[23].toString()));
			//sAddress = obj[18].toString() + " " + obj[19].toString();
			sAddress = obj[18].toString();
			sAddress1 = obj[19].toString();
			sState = obj[20].toString();
			sPin = obj[21].toString();
			custGSTNo = obj[22].toString();
			custEmailID=obj[26].toString();
			custMobileNo=obj[27].toString();
			taxCode1=obj[31].toString();
			taxCode2=obj[31].toString();
			...
			double netTotal=subTotal-discAmt;
			totalInvoiceValue = totalInvoiceValue + netTotal;
//Pooja 01-09-2021 changes for speed 
			
			
			         boolean isTaxAbatement = false; 
			         if(mapTaxForProduct.containsKey(obj[11].toString()+" "+df.format(Double.parseDouble(obj[24].toString()))))   
			         {
			        	    double cGStAmt = 0.0;
							double sGStAmt = 0.0;
							List<clsTaxBeanForInvoice> listTaxForProduct =mapTaxForProduct.get(obj[11].toString()+" "+df.format(Double.parseDouble(obj[24].toString())));
							if (listTaxForProduct.size() > 0)
							{

								for (clsTaxBeanForInvoice objBean : listTaxForProduct)
								{
									if(objBean.getDblAbatement()>0){
										isTaxAbatement=true;
									}
									if (objBean.getStrShortName().equalsIgnoreCase("CGST"))
									{
										objDtlBean.setDblCGSTPer(objBean.getDblPercent());
										objDtlBean.setDblCGSTAmt(objBean.getDblValue());
										objDtlBean.setDblTaxableAmt(objBean.getDblTaxableAmt());
										
									//	totalInvoiceValue = totalInvoiceValue + Double.parseDouble(objGST[2].toString());
									}
									else if(objBean.getStrShortName().equalsIgnoreCase("SGST"))
									{
										objDtlBean.setDblSGSTPer(objBean.getDblPercent());
										objDtlBean.setDblSGSTAmt(objBean.getDblValue());
										objDtlBean.setDblTaxableAmt(objBean.getDblTaxableAmt());
									//	totalInvoiceValue = totalInvoiceValue + Double.parseDouble(objGST[2].toString());
									}
									else if (objBean.getStrShortName().equalsIgnoreCase("IGST"))
									{
										objDtlBean.setDblIGSTPer(objBean.getDblPercent());
										objDtlBean.setDblIGSTAmt(objBean.getDblValue());
										objDtlBean.setDblTaxableAmt(objBean.getDblTaxableAmt());
									//	totalInvoiceValue = totalInvoiceValue + Double.parseDouble(objGST[2].toString());
									}
									else
									{
										objDtlBean.setDblNonGSTTaxPer(objBean.getDblPercent());
										objDtlBean.setDblNonGSTTaxAmt(objBean.getDblValue());
										objDtlBean.setDblTaxableAmt(objBean.getDblTaxableAmt());
									//	totalInvoiceValue = totalInvoiceValue + Double.parseDouble(objGST[2].toString());
									}
	...
	try
	{
		...
		hm.put("dataList", dataList);
		hm.put("bAddress", bAddress);
		hm.put("bAddress1", bAddress1);
		hm.put("bState", bState);
		hm.put("bPin", bPin);
		hm.put("sAddress", sAddress);
		hm.put("sAddress1", sAddress1);
		hm.put("sState", sState);
		hm.put("sPin", sPin);
		hm.put("totalInvoiceValueInWords", totalInvoiceValueInWords);
		hm.put("totalInvoiceValue", totalInvoiceValue);
		...
		hm.put("taxCode1", taxCode1);
		hm.put("taxCode2", taxCode2);
		// ////////////
		JasperDesign jd = JRXmlLoader.load(reportName);
		JasperReport jr = JasperCompileManager.compileReport(jd);
...
/**
 * Open Pending DC form
 * 
 * @return
 */
@RequestMapping(value = "/frmDCForInvoice", method = RequestMethod.GET)
public ModelAndView funOpenDCForInvoice(Map<String, Object> model, HttpServletRequest req)
{
	
	String custCode = req.getParameter("strCustCode");
	model.put("strCustCode", custCode);
	return new ModelAndView("frmDCForInvoice", "command", new clsReportBean());

}

@SuppressWarnings({ "rawtypes", "unused" })
@RequestMapping(value = "/loadDCForInvoice", method = RequestMethod.GET)
public @ResponseBody List funLoadDCForInvoice(HttpServletRequest request)
{
	
			String propCode = request.getSession().getAttribute("propertyCode").toString();
			String clientCode = request.getSession().getAttribute("clientCode").toString();
			String custCode = request.getParameter("strCustCode");
			List DCHelpList = new ArrayList<>();
			
			String sqlData="select  a.strDCCode,DATE_FORMAT(a.dteDCDate, '%d-%m-%Y'),a.strSOCode,b.strPName,c.strLocName, a.strPONo  " 
						   + " from tbldeliverychallanhd a,tblpartymaster b,tbllocationmaster c  "  
						   + " where a.strCustCode=b.strPCode and a.strLocCode=c.strLocCode and a.strClientCode='"+clientCode+"' " 
						   + " and  a.strClientCode=b.strClientCode and a.strClientCode=c.strClientCode and c.strPropertyCode='"+propCode+"' "   
						   + " and a.strDCCode IN (select distinct a.strDCCode from tbldeliverychallandtl a left outer join " 
						   + " (SELECT a.strSOCode AS DCCode, b.strProdCode, SUM(b.dblQty) AS DCQty " 
						   + " FROM tblinvoicehd a INNER JOIN tblinvoicedtl b ON a.strInvCode = b.strInvCode " 
						   + " and b.strClientCode='"+clientCode+"' WHERE (a.strAgainst = 'Delivery Challan') and a.strClientCode='"+clientCode+"' " 
						   + " GROUP BY a.strSOCode, b.strProdCode) b on a.strDCCode = b.DCCode and a.strProdCode = b.strProdCode " 
						   + " and a.strClientCode='"+clientCode+"'where  a.dblQty > ifnull(b.DCQty,0)) " 
						   + " and a.strCustCode='"+custCode+"' and a.strClientCode='"+clientCode+"';";
			
			DCHelpList = objGlobalFunctionsService.funGetListModuleWise(sqlData, "sql");
