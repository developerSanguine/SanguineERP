...
import com.sanguine.model.clsLocCustMasterModel_ID;
...
public class clsLocationMasterController {
	...
	public ModelAndView funOpenForm(Map<String, Object> model, HttpServletRequest request) {
		...
		clsLocationMasterBean objBean=new clsLocationMasterBean();
		String clientCode = request.getSession().getAttribute("clientCode").toString();
		request.getSession().setAttribute("formName", "frmLocationMaster");
	...
	public ModelAndView funAddUpdate(@ModelAttribute("command") @Valid clsLocationMasterBean objBean, BindingResult result, HttpServletRequest req) {
		...
		if (!result.hasErrors()) {
			...
			List<clsLocCustMasterModel> listLocCustMaster = objBean.getListLocCust();
			
			if (null != listLocCustMaster) {
				for (int counter = 0; counter < listLocCustMaster.size(); counter++) {
					clsLocCustMasterModel objLocCust = (clsLocCustMasterModel) listLocCustMaster.get(counter);
					if (null != objLocCust.getStrCustCode()) {
						clsLocCustMasterModel_ID clsLocCustMasterModel_ID = new clsLocCustMasterModel_ID(objLocCust.getStrCustCode(), objModel.getStrLocCode(), clientCode);
						objGlobal = new clsGlobalFunctions();
						
						clsLocCustMasterModel ob = new clsLocCustMasterModel(clsLocCustMasterModel_ID);
						ob.setStrLocCode(objModel.getStrLocCode());
						ob.setStrCustCode(objLocCust.getStrCustCode());
						ob.setStrCustName(objLocCust.getStrCustName());
						ob.setStrClientCode(clientCode);
						
						objLocationMasterService.funAddUpdateLocCustomer(ob);
					}
				}
			}
			
            List<clsLocCustMasterModel> listLocSupMaster = objBean.getListLocSupp();
            sqlBuilderDelete.setLength(0);
            sqlBuilderDelete.append("delete from tblloccustmaster where strLocCode='" + objModel.getStrLocCode() + "' and strClientCode='" + clientCode + "' ");
			objGlobalFunctionsService.funUpdate(sqlBuilderDelete.toString(), "sql");
			if (null != listLocSupMaster) {
				for (int counter = 0; counter < listLocSupMaster.size(); counter++) {
					clsLocCustMasterModel objLocSup = (clsLocCustMasterModel) listLocSupMaster.get(counter);
					if (null != objLocSup.getStrCustCode()) {
						clsLocCustMasterModel_ID clsLocCustMasterModel_ID = new clsLocCustMasterModel_ID(objLocSup.getStrCustCode(), objModel.getStrLocCode(), clientCode);
						objGlobal = new clsGlobalFunctions();
						
						clsLocCustMasterModel ob = new clsLocCustMasterModel(clsLocCustMasterModel_ID);
						ob.setStrLocCode(objModel.getStrLocCode());
						ob.setStrCustCode(objLocSup.getStrCustCode());
						ob.setStrCustName(objLocSup.getStrCustName());
						ob.setStrClientCode(clientCode);
	...
	public @ResponseBody List<clsLocationMasterModel> funLoadAllLocAllProp(HttpServletRequest request) {
		...
		{
			StringBuilder sqlBuilder = new StringBuilder("select b.strLocCode,b.strLocName from tbluserlocdtl a,tbllocationmaster b where  a.strLocCode=b.strLocCode AND  b.strClientCode='"+clientCode+"' AND a.strUserCode='"+userCode+"' group by a.strLocCode ");
			List list1 = objGlobalFunctionsService.funGetList(sqlBuilder.toString(), "sql");
			List list=new ArrayList<>();
			for (int i = 0; i < list1.size(); i++)
			{
			Object[] ob = (Object[]) list1.get(i);
			objBean =new clsLocationMasterBean();
			objBean.setStrLocCode(ob[0].toString());
			objBean.setStrLocName(ob[1].toString());
			list.add(objBean);
			}  
			return list;
		}
	...
	@SuppressWarnings("rawtypes")
	@RequestMapping(value = "/loadCustomerForLocationData", method = RequestMethod.GET)
	public @ResponseBody List funAssignLocCust(@RequestParam("locCode") String locCode, HttpServletRequest req) {
		objGlobal = new clsGlobalFunctions();
		String clientCode = req.getSession().getAttribute("clientCode").toString();
		StringBuilder sqlBuilder = new StringBuilder();
		
		List<clsLocCustMasterModel> listCust = new ArrayList<clsLocCustMasterModel>();
	    sqlBuilder.setLength(0);
	    //sqlBuilder.append("select strLocName,strLocCode from clsLocationMasterModel where strClientCode='" + clientCode + "' " + "and strPropertyCode='" + propCode + "'");
	    sqlBuilder.append("SELECT strCustCode,strCustName FROM clsLocCustMasterModel  WHERE strLocCode='"+locCode+"' AND strClientCode='"+clientCode+"' ");
	    List list = objGlobalFunctionsService.funGetList(sqlBuilder.toString(), "hql");
	    for (int i = 0; i < list.size(); i++) {
			Object[] ob = (Object[]) list.get(i);
			clsLocCustMasterModel objLocCustomer = new clsLocCustMasterModel();
			objLocCustomer.setStrCustCode(ob[0].toString());
			objLocCustomer.setStrCustName(ob[1].toString());
			
			listCust.add(objLocCustomer);
		}
		return listCust;
	}
