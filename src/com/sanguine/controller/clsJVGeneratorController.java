	...
	public String funGenrateJVforGRN(String GRNCode, String clientCode, String userCode, String propCode, HttpServletRequest req) {
		...
		if(null!=list && list.size()>0)
		{
			...
			// jvdtl entry Start
				...
				/*String sqlAdj ="select a.strGRNCode,a.dblAdjustAmt,b.strAccountCode,b.strMasterDesc,date(a.dtGRNDate),a.strRemarks ,"
						+ " date(a.dtGRNDate),'','' from	tblbillpassdtl a, tbllinkup b  where a.strReasonCode =b.strMasterCode					"
						+ " and a.strGRNCode='" + objModel.getStrGRNCode() + "' and a.strClientCode='" + objModel.getStrClientCode() + "' ;";
				*/
				String sqlGRNDtl = " select a.strGRNCode,a.dblTotal - c.dblAdjustAmt,b.strDebtorCode,b.strPName,date(a.dtGRNDate),a.strNarration ,date(a.dtDueDate),a.strBillNo,b.strPCode "
					+ " from tblgrnhd a,tblpartymaster b,tblbillpassdtl c "
					+ " where a.strSuppCode =b.strPCode and a.strGRNCode=c.strGRNCode and a.strGRNCode='" + objModel.getStrGRNCode() + "' and a.strClientCode='" + objModel.getStrClientCode() + "' "
				    + " UNION  "
				    + " select a.strGRNCode,a.dblAdjustAmt,b.strAccountCode,b.strMasterDesc,date(a.dtGRNDate),a.strRemarks ,"
					+ " date(a.dtGRNDate),'ADJUSTMENT',a.strReasonCode from	tblbillpassdtl a, tbllinkup b  where a.strReasonCode =b.strMasterCode					"
					+ " and a.strGRNCode='" + objModel.getStrGRNCode() + "' and a.strClientCode='" + objModel.getStrClientCode() + "'" ;
				...
				if (null != listSupplier) {
					for (int i = 0; i < listSupplier.size(); i++) {
						...
						if(ob[7].toString().equalsIgnoreCase("ADJUSTMENT")){
							listCustSuppAccDetails=objGlobalFunctions.funGetCustSupplierAccountDetails("Reason",ob[8].toString() , clientCode, propCode);
						...
						objJVDtlBean.setDblCreditAmt(Double.parseDouble(ob[1].toString()));
						if(supplierType.equals("Foreign"))
						{
						...
						if(ob[7].toString().equalsIgnoreCase("ADJUSTMENT")){
							objJVDtlBean.setStrNarration(ob[5].toString());
						}else{
