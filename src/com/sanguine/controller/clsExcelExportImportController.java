...
public class clsExcelExportImportController {
	...
	public ModelAndView funPhyStkPstExcelExport(HttpServletRequest request) {
		...
		{
			header="Group Name, SubGroupName,ProductCode,ProductName,Stock,Qty,UOM,Price";
		}
		else{
			header="Group Name, SubGroupName,ProductCode,ProductName,Qty,UOM,Price";
		}
	...
	private List funLocMastReOrderLvl(HSSFSheet worksheet, HttpServletRequest request) {
		...
		try {
			...
			while (i <= worksheet.getLastRowNum()) {
				...
				HSSFRow row = worksheet.getRow(i++);
				// Sets the Read data to the model class
				String ReOrderlvl = String.valueOf(row.getCell(7).getStringCellValue());
				String ReOrderQty = String.valueOf(row.getCell(8).getStringCellValue());
				if (!ReOrderlvl.equals("") || !ReOrderQty.equals("")) {
					...
					ReorderLvlList.add(row.getCell(7).getStringCellValue());
					ReorderLvlList.add(row.getCell(8).getStringCellValue());
					// Sends the model object to service layer for validation,
					// data processing and then to persist
	...
	public List funGuestList(HSSFSheet worksheet, HttpServletRequest request) {
		...
		try {
			...
			while (i <= worksheet.getLastRowNum()) {
				...
				{
				String mobNo = (row.getCell(5).toString()); 
