...
import com.sanguine.model.clsAllProductModel;
...
import com.sanguine.controller.clsGRNController;
@Controller
public class clsSupplierMasterController {
	...
	public void initBinder(WebDataBinder binder) {
	    binder.setAutoGrowCollectionLimit(10000000);
	}
	...
	public ModelAndView funOpenForm(Map<String, Object> model, HttpServletRequest request) {
		...
		request.getSession().setAttribute("formName", "frmSupplierMaster");
	...
	public ModelAndView funAddUpdate(@ModelAttribute("command") @Valid clsSupplierMasterBean objBean, BindingResult result, HttpServletRequest req) {
		...
		if (!result.hasErrors()) {
			...
			clsAllProductModel reOrderModel = new clsAllProductModel();
		//	funAddProdSuppMaster(clientCode,objModel.getStrPCode(),userCode,reOrderModel.getStrProdCode());
			/*List<clsProdSuppMasterModel> listAllProd = objBean.getListAllProd();
			if (null != listAllProd) {
			objGlobal = new clsGlobalFunctions();
				clsProdSuppMasterModel objProdSuppdata = new clsProdSuppMasterModel();
				objGRNService.funDeleteProdSupp(objProdSuppdata.getStrSuppCode(),objProdSuppdata.getStrProdCode(), clientCode);
				for (int counter = 0; counter < listAllProd.size(); counter++) {
				objProdSuppdata = (clsProdSuppMasterModel) listAllProd.get(counter);
				clsProdSuppMasterModel_ID clsProdSuppMasterModel_ID = new clsProdSuppMasterModel_ID(objProdSuppdata.getStrProdCode(), objModel.getStrPCode());
				clsProdSuppMasterModel ob = new clsProdSuppMasterModel(clsProdSuppMasterModel_ID);
				if(objProdSuppdata.getStrProdCode()!=null)
					   
				{
                        	ob.setStrSuppCode(objModel.getStrPCode());
							ob.setStrProdCode(objProdSuppdata.getStrProdCode());
							ob.setStrClientCode(clientCode);
							ob.setDblMaxQty(0.00);
							ob.setStrDefault("Y");
							ob.setStrLeadTime("");
							ob.setStrSuppPartDesc("");
							ob.setStrSuppPartNo("");
							ob.setStrSuppUOM("");
							ob.setDblLastCost(objProdSuppdata.getDblLastCost());
							ob.setDtLastDate(objGlobalFunctions
    								.funGetCurrentDate("yyyy-MM-dd"));
						    ob.setDteInstallation("1900-01-01 00:00:00");
    						StringBuilder sqlBuilder = new StringBuilder();
    						sqlBuilder.setLength(0);
    						sqlBuilder
    								.append("update tblprodsuppmaster set strDefault='N' where  strProdCode='"
    										+ objModel.getStrPCode()
    										+ "' and strClientCode='"
    										+ clientCode
    										+ "'");
    						objGlobalFunctionsService.funUpdate(sqlBuilder.toString(), "sql");
    						objProdSuppService.funAddUpdateProdSupplier(ob);
                        	}
				}
			}
			...
			*/
			...
			List<clsProdSuppMasterModel> arrListProdSupp = objBean.getListAllProd();
			if (arrListProdSupp != null) {
				...
				for (int i = 0; i <=5 ; i++) {
					...
					{
					objProdSuppService.funDeleteSuppProds(Obj.getStrProdCode(), clientCode);
...
//					if (Obj.getDblStandingOrder() != 0) {
						...
						objProdSuppService.funAddUpdateProdSupplier(Obj);
					}
//					}
				...
				*/
				...
				/*
				for (int i = 0; v <= 7000; v++) {
					clsProdSuppMasterModel Obj = arrListProdSupp.get(v);
...
//					}
				...
				*/
				...
				clsProdSuppMasterModel Obj = arrListProdSupp.get(3400);
...
//				if (Obj.getDblStandingOrder() != 0) {
					...
					System.out.println(3400 + "    " + Obj.getStrProdCode() + "===");
					objProdSuppService.funAddUpdateProdSupplier(Obj);
				}
	...
	private boolean funAddProdSuppMaster( String clientCode,
		 String suppCode, String userCode,String prodCode) {
		boolean flgInsert = false;

			clsGRNDtlModel ob = null;
			...
			objProdSupp.setStrProdCode(suppCode);
			...
			objProdSupp.setDblLastCost(objProdSupp.getDblLastCost());
	...
	public @ResponseBody List<clsAllProductModel> funAssignAllLocation(HttpServletRequest req) {
		String clientCode = req.getSession().getAttribute("clientCode").toString();
		String prodCode = req.getParameter("prodCode").toString();
		String sql = " SELECT a.strProdCode,a.strProdName,a.dblCostRM FROM tblproductmaster a ";
	...
	//	List LocationMasterList = objLocationMasterService.funGetdtlList(prodCode, clientCode);
		List<clsAllProductModel> list = new ArrayList<clsAllProductModel>();
		for (int i = 0; i < arrList.size(); i++) {
			Object[] ob = (Object[]) arrList.get(i);
			clsAllProductModel reOrderModel = new clsAllProductModel();
			reOrderModel.setStrProdCode(ob[0].toString());
			reOrderModel.setStrProductName(ob[1].toString());
			reOrderModel.setDblCostRM(Double.valueOf(ob[2].toString()));
	...
	@SuppressWarnings("rawtypes")
	@RequestMapping(value = "/loadProductForSupp", method = RequestMethod.GET)
	public @ResponseBody List funAssignFieldsForProductSupp(@RequestParam("partyCode") String PCode, HttpServletRequest req) {
		String clientCode = req.getSession().getAttribute("clientCode").toString();
		String sql =" select a.strProdCode,b.strProdName,a.dblLastCost from tblprodsuppmaster a , tblproductmaster b,"
				   + " tblpartymaster c where a.strProdCode=b.strProdCode AND a.strSuppCode=c.strPCode and "
				   + " a.strSuppCode='"+PCode+"' AND a.strClientCode= '"+clientCode+"'"
	...
	private ModelAndView funReport(@ModelAttribute("command") clsReportBean objBean, HttpServletResponse resp, HttpServletRequest req) {
		 if(objBean.getStrDocType().equalsIgnoreCase("xls"))
				
			{
			     ModelAndView mod = funCallExcelReportDump(objBean, resp, req);
				 return mod; 
	...
	private ModelAndView funCallExcelReportDump(@ModelAttribute("command") clsReportBean objBean, HttpServletResponse resp, HttpServletRequest req) {
		...
		String header = "ProductName,PartyName,GRNDate,GRNCode,Qty,UOM,Rate,Value, ";

		List exportList = new ArrayList();
		
		exportList.add("ProductWiseSupplierRateHistory_");
		
		List titleData = new ArrayList<>();
		titleData.add("Product Wise Supplier Rate History");
		...
		String sql= " select b.strProdCode, d.strProdName, c.strPName, a.strAgainst , a.strPONo,  DATE_FORMAT(date(a.dtGRNDate),'%d-%m-%Y') as dtGRNDate, " + " a.strGRNCode, b.dblQty, d.strReceivedUOM, b.dblUnitPrice, b.dblTotalPrice  " + " from tblgrnhd a, tblgrndtl b, tblpartymaster c, tblproductmaster d " + " where a.strGRNCode = b.strGRNCode and a.strSuppCode = c.strPCode "
				+   " and b.strProdCode = d.strProdCode and a.dtGRNDate >= '" + frmDate + "' and a.dtGRNDate <= '" + ToDate + "' " + " and a.strClientCode ='" + clientCode + "' and b.strClientCode = '" + clientCode + "' and c.strClientCode = '" + clientCode + "' and d.strClientCode = '" + clientCode + "'";
		...
		sql+=  " order by d.strProdName, a.dtGRNDate, a.strGRNCode";
	   ...
	   for (int i = 0; i < list.size(); i++) {
			...
			dataList.add(Double.parseDouble(ob[9].toString())); 
		    dataList.add(df.format(Double.parseDouble(ob[10].toString())));
			
		    openingStklist.add(dataList);
		    
		    
			totalQty=totalQty + (Double.parseDouble(ob[7].toString()));

			totalRate=totalRate + (Double.parseDouble(ob[9].toString()));

			totalAmt = totalAmt + (Double.parseDouble(ob[10].toString()));
		...
		totalRow.add("");
		totalRow.add("");
		totalRow.add(totalQty);
		totalRow.add("");
		totalRow.add(totalRate);
