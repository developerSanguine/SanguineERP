...
public class clsGlobalFunctionsDaoImpl implements clsGlobalFunctionsDao {
	...
	public Map<String, String> funGetUserWisePropertyList(String clientCode, String userCode, String userPrpoerty) {
		...
		try {
			if (!userPrpoerty.equals("ALL")) {
				...
				String hql = "from clsPropertyMaster where strClientCode='" + clientCode + "'  order by strPropertyCode ";
	...
	public HashMap<String, String> funGetLocationList(String propCode, String clientCode) {
		String customerCode= req.getSession().getAttribute("usercode").toString();
		...
		try {
			...
			{
				String sql = "SELECT b.strLocCode,b.strLocName from tbluserlocdtl a,tbllocationmaster b where  a.strLocCode=b.strLocCode AND  b.strClientCode='"+clientCode+"' AND a.strUserCode='"+customerCode+"' group by a.strLocCode ";
				Query query = sessionFactory.getCurrentSession().createSQLQuery(sql);
				List listLocation = query.list();
				for (int cnt = 0; cnt < listLocation.size(); cnt++) {
					Object[] arrObjLocation = (Object[]) listLocation.get(cnt);
					map.put(arrObjLocation[0].toString(), arrObjLocation[1].toString());
				}
